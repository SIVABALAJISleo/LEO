  ...
  const modules = [
    {
      ...
      enabled: true,
      config: { precision: "int8", dynamic: true },
      settings: { target_speedup: 2.0 },
    },
    {
      module_name: "pruning",
      module_type: "compression",
      enabled: true,
      config: { sparsity: 0.5, structured: true },
      settings: { sensitivity_analysis: true },
    },
    {
      module_name: "distillation",
      module_type: "knowledge_transfer",
      enabled: false,
      config: { teacher_model: null, temperature: 2.0 },
      settings: { progressive: false },
    },
    {
      module_name: "fusion",
      module_type: "graph_optimization",
      enabled: true,
      config: { aggressive: false, preserve_accuracy: true },
    ...
    },
    {
      ...
      module_type: "memory",
      enabled: true,
      config: { max_size_mb: 4096, ttl_seconds: 3600 },
      settings: { preload: true },
    },
    {
      module_name: "batching",
      module_type: "throughput",
      enabled: true,
      config: { max_batch_size: 32, timeout_ms: 100 },
      settings: { dynamic_sizing: true },
    },
    {
      module_name: "parallel_inference",
      module_type: "compute",
      enabled: true,
      config: { num_workers: 4, load_balancing: "round_robin" },
    ...
    },
    {
      ...
      module_type: "memory",
      enabled: true,
      config: { gradient_checkpointing: true, offload: false },
      settings: { aggressive_gc: true },
    },
  ];
  ...
  const { error } = await supabase.from("module_configs").upsert(
    modules.map((m) => ({
      ...
      compression_ratio_achieved:
        m.module_type === "compression" && m.enabled ? 0.3 + Math.random() * 0.4 : null,
    })),
...
async function seedModuleStatus(supabase: any, userId: string) {
  ...
  const { error } = await supabase.from("module_status").upsert(
    statuses.map((s) => ({
      user_id: userId,
      ...s,
...
async function seedGpuSystemStatus(supabase: any) {
  ...
  return {
    success: !error,
    error: error?.message,
    notice: "Worker registered as offline - awaiting real agent",
  };
}
...
async function seedModels(supabase: any, userId: string) {
  ...
  const models = [
    ...
    },
    {
      name: "BERT-Distilled",
      model_type: "transformer",
      description: "Efficient BERT variant for classification",
      version: "2.1.0",
      status: "active",
      is_public: false,
    },
    ...
    },
    {
      ...
      model_type: "transformer",
      description: "Optimized Llama 3 for inference",
      version: "3.0.0",
      status: "active",
      is_public: false,
    },
