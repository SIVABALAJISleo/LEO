  ...
  try {
    ...
    if (req.method === "GET" && (path === "" || path === "/")) {
      ...
      const avgLatency = perfMetrics?.length
        ? Math.round(
            perfMetrics.reduce((sum, m) => sum + (m.latency_ms || 0), 0) / perfMetrics.length,
          )
        : 120;
      ...
      const response: SystemStatus = {
        ...
        features: [
          ...
          { name: "API Access", status: "available" },
          {
            ...
            status: status === "major_outage" ? "unavailable" : "available",
          },
        ],
        metrics: {
...
`.trim();
    ...
    if (req.method === "GET" && path === "/boundaries") {
      const boundaries = {
        ...
        trustStatement:
          "This platform executes everything software is allowed to execute, and formally integrates everything software is not allowed to execute.",
        coverageMetrics: {
        ...
        boundaries: [
          {
            ...
            category: "physics",
            neverDo: "Train frontier AI models",
            reason: "Requires physical GPU clusters",
          },
          {
            id: "no_legal_decisions",
            category: "law",
            neverDo: "Make binding legal decisions",
            reason: "Requires human/court authority",
          },
          {
            id: "no_medical_diagnosis",
            category: "ethics",
            neverDo: "Provide final medical diagnoses",
            reason: "Requires licensed practitioners",
          },
          {
            id: "no_safety_override",
            category: "ethics",
            neverDo: "Override safety-critical systems",
