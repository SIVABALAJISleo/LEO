...
foreach ($dir in $targetDirs) {
    ...
    if (Test-Path $dirPath) {
        ...
        foreach ($f in $filesToRename) {
            $newName = $f.Name -creplace 'project_leo', 'project_hyper' -creplace 'leo_', 'hyper_' -creplace '_leo', '_hyper' -creplace 'leo', 'hyper'
...
foreach ($dir in $targetDirs) {
    ...
    if (Test-Path $dirPath) {
        ...
        foreach ($d in $dirsToRename) {
            $newName = $d.Name -creplace 'project_leo', 'project_hyper' -creplace 'leo', 'hyper'
