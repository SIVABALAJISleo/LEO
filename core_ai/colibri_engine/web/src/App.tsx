...
export default function App() {
  ...
  const [baseUrl, setBaseUrl] = useState(() => {
    ...
    if (servedByEngine && saved === "http://127.0.0.1:8000/v1" && defaultBase !== saved)
      return defaultBase;
    return saved;
  });
  ...
  const [model, setModel] = useState(() =>
    stored(localStorage, "colibri.model", "glm-5.2-colibri"),
  );
  ...
  const send = async () => {
    ...
    try {
      const result = await streamChat({
        ...
        onDelta: (delta) => {
          if (firstToken) {
            ...
            setStreamStart(performance.now());
            firstToken = false;
          }
          count++;
          setTokenCount(count);
      ...
      if (controller.signal.aborted) {
        updateMessages((current) =>
          current.filter((item) => item.id !== assistant.id || item.content),
        );
      } else {
    ...
    <div className="app-shell">
      <aside className="sidebar">
        <div className="brand-row">
          ...
          <div>
            <h1>colibrì</h1>
            <p>local giant, tiny footprint</p>
          </div>
        </div>

        <section className="side-section">
          <div className="section-title">
            <Link2 className="size-3.5" /> Connection
          </div>
          <label>
            API endpoint
          ...
          <label>
            ...
            <span className="field-help">Kept in memory only · sent to this endpoint</span>
          </label>
          ...
          <div className={cn("connection-state", connected && "connected")} aria-live="polite">
            <span />
            {connected ? "Engine reachable" : "Not connected"}
          </div>
        </section>

        <section className="side-section runtime-section" aria-live="polite">
          <div className="section-title">
            <Activity className="size-3.5" /> Runtime
          </div>
          {health?.hwinfo ? (
            <div className="hw-panel">
              ...
              {health.hwinfo.gpus > 0 ? (
                <div className="hw-row">
                  ...
                  <span>
                    {health.hwinfo.gpus}× GPU
                    <small>{health.hwinfo.vram_total_gb.toFixed(0)} GB VRAM</small>
                  </span>
                </div>
              ) : null}
              <div className="hw-row">
            ...
            <>
                    ...
                    return (
                      <div className="tier-panel">
                        <div
                          className="tier-bar"
                          role="img"
                          aria-label={`Experts: ${t.vram} VRAM, ${t.ram} RAM, ${t.disk} disk`}
                        >
                          <span
                            className="tier-vram"
                            style={{ width: `${(100 * t.vram) / total}%` }}
                          />
                          <span
                            className="tier-ram"
                            style={{ width: `${(100 * t.ram) / total}%` }}
                          />
                          <span
                            className="tier-disk"
                            style={{ width: `${(100 * t.disk) / total}%` }}
                          />
                        </div>
                        <div className="tier-legend">
                          <span>
                            <i className="tier-vram" />
                            VRAM <strong>{t.vram.toLocaleString()}</strong>
                            <small>{t.vram_gb.toFixed(1)} GB</small>
                          </span>
                          <span>
                            <i className="tier-ram" />
                            RAM <strong>{t.ram.toLocaleString()}</strong>
                            <small>{t.ram_gb.toFixed(1)} GB</small>
                          </span>
                          <span>
                            <i className="tier-disk" />
                            Disk <strong>{t.disk.toLocaleString()}</strong>
                          </span>
                        </div>
                      </div>
                    );
              ...
              {totalTokens.prompt + totalTokens.completion > 0 ? (
                <div className="session-stats">
                  <span>
                    <Database className="size-3" /> Session:{" "}
                    <strong>{totalTokens.prompt.toLocaleString()}</strong> prompt +{" "}
                    <strong>{totalTokens.completion.toLocaleString()}</strong> completion
                  </span>
                </div>
              ) : null}
              <div className="runtime-foot">
                <span className="runtime-dot" /> Scheduler online <code>{kvSlots} KV</code>
              </div>
            </>
          ) : (
            <p className="runtime-unavailable">
              {connected
                ? healthError || "Runtime metrics unavailable"
                : "Probe the server to inspect runtime state."}
            </p>
          )}
        </section>
        ...
        <section className="side-section">
          <div className="section-title">
            <SlidersHorizontal className="size-3.5" /> Inference
          </div>
          <label>
            Model
          ...
          {health?.kv_slots && health.kv_slots > 1 ? (
            <label>
              ...
              <span className="field-help">
                Isolated context · conversation follows the selected slot
              </span>
            </label>
          ) : null}
          <label>
            ...
            <Input
              ...
              onChange={(event) => {
                ...
                if (Number.isFinite(value))
                  setMaxTokens(Math.min(4096, Math.max(1, Math.round(value))));
              }}
            />
          </label>
          <button
          ...
          >
            <span>
              <BrainCircuit className="size-4" /> Reasoning
            </span>
            <i>
              <b />
            </i>
        ...
        <div className="sidebar-foot">
          <Cpu className="size-3.5" />
          <span>OpenAI-compatible transport</span>
        </div>
      </aside>
      ...
      <main className="chat-panel">
        <header className="topbar">
          ...
          <div className="view-tabs">
            ...
            <button className={view === "brain" ? "active" : ""} onClick={() => setView("brain")}>
              <BrainCircuit className="size-3.5" /> Brain
            </button>
          </div>
          <div className="top-actions">
            {loading && tokenCount > 0 ? (
              <Badge className="badge-live">
                <Zap className="size-3 flash" /> {tokenCount} tokens
              </Badge>
            ) : null}
            {!loading && tokPerSec != null ? (
              <Badge className="badge-speed">
                <Gauge className="size-3" /> {tokPerSec.toFixed(1)} tok/s
              </Badge>
            ) : null}
            {!loading && ttft != null ? (
              <Badge>
            ...
            {!loading && lastRun?.usage ? (
              <Badge>
                <Layers className="size-3" /> {lastRun.usage.prompt_tokens}→
                {lastRun.usage.completion_tokens}
              </Badge>
            ) : null}
            {lastRun?.queueWaitMs != null ? (
              <Badge>
                <Clock className="size-3" /> queue {Math.round(lastRun.queueWaitMs)}ms
              </Badge>
            ) : null}
            <Badge>
              <MonitorDot className="size-3" /> slot {cacheSlot + 1}
            </Badge>
            <Button
              variant="ghost"
              size="sm"
              ...
              {!messages.length ? (
                <div className="empty-state">
                  ...
                  <h2>
                    Ask the giant.
                    <br />
                    <em>Keep the machine yours.</em>
                  </h2>
                  <p>
                    Connect to a local colibrì server and stream responses directly from your
                    hardware. Nothing leaves the endpoint you choose.
                  </p>
                  <div className="suggestions">
                    {[
                      "Explain how expert routing works",
                      "Write a small C benchmark",
                      "Compare RAM and VRAM caching",
                    ].map((item) => (
                  ...
                  {messages.map((item) => (
                    <article key={item.id} className={cn("message", item.role)}>
                      <div className="avatar">
                        {item.role === "user" ? "Y" : <Feather className="size-4" />}
                      </div>
                      <div>
            ...
            <div className="composer-wrap">
              ...
              <div className="composer">
                ...
                <div className="composer-foot">
                  <span>
                    <MessageSquareText className="size-3.5" /> Enter to send · Shift+Enter for
                    newline
                  </span>
                  {loading ? (
                    <Button
