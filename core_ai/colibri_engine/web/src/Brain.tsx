...
export function Brain({
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        const res = await fetch(endpoint(base, "/experts"), {
        ...
        if (next.seq !== lastSeq.current && next.hits) {
          ...
          if (!pulseRef.current || pulseRef.current.length !== n)
            pulseRef.current = new Float32Array(n);
        ...
        /* engine busy or restarting — keep the last frame */
      }
    };
    void poll();
  ...
  const totals = data
    ? (() => {
        ...
        for (let i = 0; i < data.rows * data.cols; i++)
          t[(parseInt(data.map.substr(i * 2, 2), 16) || 0) >> 6]++;
        return t;
      })()
    : [0, 0, 0];
    ...
    <div className="brain-page">
      <div className="brain-head">
        <div className="section-title">
          <BrainCircuit className="size-4" /> Expert Cortex —{" "}
          {data ? `${data.rows} layers × ${data.cols} experts` : "waiting for engine"}
        </div>
        <div className="brain-legend">
          <span>
            <i style={{ background: "#4ed6a5" }} /> VRAM {totals[2].toLocaleString()}
          </span>
          <span>
            <i style={{ background: "#5a9bd8" }} /> RAM {totals[1].toLocaleString()}
          </span>
          <span>
            <i style={{ background: "#3a4750" }} /> Disk {totals[0].toLocaleString()}
          </span>
          <span>
            <Flame className="size-3" /> brightness = routing heat
          </span>
      ...
      <div className="brain-canvas-wrap" ref={wrapRef}>
        ...
        {!connected && (
          <p className="runtime-unavailable">Connect to the engine to see the cortex.</p>
        )}
      </div>
      {tip &&
        data &&
        (() => {
          ...
          return (
            <div className="brain-tip" style={{ left: tip.x + 14, top: tip.y + 14 }}>
              <div className="brain-tip-title">
                <Layers className="size-3" /> Layer {realLayer}
                {isMtp ? " (MTP)" : ""} · Expert {tip.col}
              </div>
              <div>
                Tier:{" "}
                <strong style={{ color: ["#8b9aa3", "#5a9bd8", "#4ed6a5"][tip.tier] }}>
                  {TIER_NAME[tip.tier]}
                </strong>
              </div>
              <div>
                Heat:{" "}
                <strong>{tip.heat === 0 ? "never routed" : `~2^${tip.heat} selections`}</strong>
              </div>
              {entry ? (
                <>
                  <div
                    className={entry.label.startsWith("specialist") ? "brain-tip-spec" : undefined}
                  >
                    {entry.label.startsWith("specialist")
                      ? `⭐ Specialist: ${entry.top}`
                      : "Generalist"}
                    <small> (entropy {entry.entropy})</small>
                  </div>
                  <div className="brain-tip-aff">
                    {Object.entries(entry.affinity)
                      .sort((a, b) => b[1] - a[1])
                      .slice(0, 3)
