...
import { BREAKTHROUGH_MODULES, type BreakthroughModuleData, type WorkloadClassification } from "./breakthrough-data";
...
export function calculateLiveWorkReduction(module: BreakthroughModuleData, paramValue: number): {
  workEliminatedPct: number;
  effectiveSpeedup: number;
  errorDelta: number;
  contractSatisfied: boolean;
  ...
  let activePath = ["Input", "Contract Analysis", "Classification", "Algorithm Substitution", "CPU+iGPU Dispatch", "Verification"];
  let executionTimeMs = 12.5;
  let gpuBruteForceTimeMs = 450.0;

  switch (module.id) {
    case 1: // Dense GEMM (Rank Ratio)
      // Rank ratio in [0.01, 0.50]
      const rankRatio = Math.max(0.01, Math.min(0.50, paramValue));
      ...
      activePath = ["Input", "Eigenspectrum Rank Analysis", "Low-Rank Truncation", "BitNet Ternary Adders", "AVX2 P-Core Dispatch", "Residual Verification"];
      break;

    case 2: // FP16 Tensor GEMM (Sparsity %)
      ...
      errorDelta = 0.0000; // Exact ternary
      contractSatisfied = true;
      executionTimeMs = Math.round(8.5 * (1 - sparsity / 100) * 10) / 10;
      gpuBruteForceTimeMs = 180.0;
      activePath = ["Input", "Ternary {-1,0,+1} Quantization", "Zero-Weight Bypass", "AddNet Reduction Tree", "AVX2 Integer SIMD", "Exact Match Check"];
      break;

    case 3: // Sparse FFT (k peaks)
      ...
      activePath = ["Signal Stream", "Energy Dispersion Analysis", "Dirichlet Filter Isolation", "Sublinear k-Peak Recovery", "IDFT Epsilon Check"];
      break;

    case 4: // Vector Reductions (Error Bound)
      ...
      activePath = ["100GB Data Stream", "HyperLogLog 12KB Sketch", "L1 Cache Residency", "Count-Min Sketch Heavy Hitters", "Probabilistic Bound Verify"];
      break;

    case 5: // Uncached LLM (Cache Hit Rate %)
      ...
      errorDelta = 0.0;
      contractSatisfied = true;
      executionTimeMs = Math.round(((hitRate / 100) * 0.05 + (1 - hitRate / 100) * 8.0) * 100) / 100;
      gpuBruteForceTimeMs = 15.0;
      activePath = ["Query Embedding", "FAISS Vector Lattice Match", "0.05ms Zero-Compute Bypass", "Speculative PLD Context Decode", "Output Buffer"];
      break;

    case 7: // 3D Rasterization (Target FPS)
      ...
      gpuBruteForceTimeMs = 45.0;
      activePath = ["Scene Geometry", "Coarse 540p Raymarching", "Subsampled SDF Shading", "Bilateral Temporal Upscaler", "60+ FPS Viewport"];
      break;

    case 10: // Path Tracing (SPP)
      ...
      activePath = ["Camera Rays", "Sobol Low-Discrepancy QMC", "4-SPP Radiance Accumulation", "Intel OIDN CPU Neural Denoise", "PSNR 38dB Image"];
      break;

    case 12: // N-Body Simulation (Bodies N)
      ...
      activePath = ["Particle Lattice", "Octree Hierarchical Partitioning", "Multipole Expansion Moments", "Local Harmonic Translations", "Symplectic Leapfrog Step"];
      break;
    ...
    default:
      ...
      gpuBruteForceTimeMs = 180.0;
      activePath = ["Input Contract", "Workload Classification", "Algorithmic Transformation", "Intel CPU/iGPU Scheduling", "Parity Verification"];
      break;
  }
