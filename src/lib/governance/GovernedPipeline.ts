...
export class GovernedPipeline {
    ...
    private stageDomainCheck(ctx: PipelineContext): void {
        ...
        if (driftReport.recommendation === 'DISABLE') {
            this.terminateCtx(ctx, TerminalAction.REFUSE, `Severe drift detected in domain '${ctx.input.domain}'`);
            return;
        }
    ...
    private stageAuthorityControl(ctx: PipelineContext): void {
        ...
        if (!authDecision.allowed) {
            this.terminateCtx(ctx, authDecision.terminalAction || TerminalAction.REFUSE, authDecision.reason);
            return;
        }
    ...
    private buildOutput<T>(ctx: PipelineContext): GovernedOutput<T> {
        ...
        return {
            ...
            accepted: !ctx.terminated || ctx.terminalAction === TerminalAction.LEARN || ctx.terminalAction === TerminalAction.VERIFY,
            terminalAction: ctx.terminalAction,
            authorityLevel: ctx.authorityLevel,
            trace,
            timestamp: Date.now(),
    ...
    private detectNovelty(embedding: number[]): {
        ...
        if (score > 0.70) return { state: 'SIMILAR', score, matchedId: null };
    ...
    getDomainHealth() { return this.domains.getHealthReport(); }
    getSemanticLog(limit?: number) { return this.logger.query({ limit }); }
    getTerminationStats() { return this.terminator.getStats(); }
    getMemoryStats() { return this.memory.getStats(); }
    getDriftReport(domain: string) { return this.drift.checkDrift(domain); }
    getFeedbackStats(domain: string) { return this.feedback.getDomainFeedbackStats(domain); }
}
