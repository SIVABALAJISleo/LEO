...
class ForcedRealityExercise {
  ...
  isFullyCovered(): boolean {
    return Array.from(this.requirements.values())
      .every(r => r.successCount >= r.minimumExecutions);
  }
  ...
  getMissingExercises(): ExerciseRequirement[] {
    return Array.from(this.requirements.values())
      .filter(r => r.successCount < r.minimumExecutions);
  }
  ...
  generateCoverageReport(): {
    ...
    const missingProofs = categoriesStatus
      ...
      .map(c => `${c.category}: ${c.status === 'failed' ? 'Last execution failed' : 'Never executed'}`);
  ...
  private updateCoverageStats(): void {
    const covered = Array.from(this.requirements.values())
      .filter(r => r.successCount >= r.minimumExecutions).length;
