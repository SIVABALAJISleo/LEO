...
class WorkloadClassifierEngine {
  ...
  classify(
    ...
    const classification: WorkloadClassification = {
      ...
      downscaleable: constraints.allowDownscale !== false && !categories.includes('precision_critical'),
      delegatable: !categories.includes('latency_critical') || (constraints.maxLatencyMs || 0) > 5000,
      confidence: 0.90, // Fixed confidence - no random values
  ...
  private isGpuRequired(workloadType: string, categories: WorkloadCategory[]): boolean {
    const gpuWorkloads = [
      'training', 'inference', 'render', 'image_generation', 
      'video_encode', 'video_decode', 'llm', 'diffusion',
      '3d_render', 'ray_trace', 'neural'
    ];
