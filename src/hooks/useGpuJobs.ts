...
export function useGpuJobs() {
  ...
  const createJob = useCallback(async (input: CreateGpuJobInput): Promise<GpuJob | null> => {
    if (!user) return null;
    ...
    try {
      ...
      orchestrator.execute('ai_inference', { query: newJob.job_name })
        .then(result => {
  ...
  return {
    ...
    getMemoryReport,
    getThermalStatus: () => ({ gpu_temp: 45, cpu_temp: 48, is_safe: true, is_throttled: false, recommended_action: 'continue' }),
    getQueuePosition: () => 1,
    getJobStats,
    refreshJobs: () => { },
