...
export const useCollaborationData = (sessionId?: string) => {
  ...
  const addComment = async (content: string, moduleName?: string, parentId?: string, mentions?: string[]) => {
    if (!user || !sessionId) return;
  ...
  const recordChange = async (changeType: string, targetModule: string, previousValue: Json, newValue: Json) => {
    if (!user || !sessionId) return;
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'collaboration_participants', filter: `session_id=eq.${sessionId}` }, () => fetchSessionData())
      .on('postgres_changes', { event: '*', schema: 'public', table: 'collaboration_changes', filter: `session_id=eq.${sessionId}` }, () => fetchSessionData())
      .on('postgres_changes', { event: '*', schema: 'public', table: 'collaboration_comments', filter: `session_id=eq.${sessionId}` }, () => fetchSessionData())
      .on('postgres_changes', { event: '*', schema: 'public', table: 'collaboration_messages', filter: `session_id=eq.${sessionId}` }, () => fetchSessionData())
      .on('postgres_changes', { event: '*', schema: 'public', table: 'module_locks', filter: `session_id=eq.${sessionId}` }, () => fetchSessionData())
      .subscribe();
