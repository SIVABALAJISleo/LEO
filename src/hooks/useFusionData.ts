...
export function useFusionData() {
  ...
  const createFusedModel = async (data: { name: string; fusion_strategy: string; description?: string }) => {
    if (!user) return;
    const { error } = await supabase.from('fused_models').insert({ ...data, user_id: user.id });
    if (error) toast.error('Failed to create model');
    else { toast.success('Fused model created'); fetchAll(); }
  };

  const createStrategy = async (data: { name: string; strategy_type: string; conflict_resolution?: string }) => {
    if (!user) return;
    const { error } = await supabase.from('fusion_strategies').insert({ ...data, user_id: user.id });
    if (error) toast.error('Failed to create strategy');
    else { toast.success('Strategy created'); fetchAll(); }
  };
  ...
  return { fusedModels, strategies, performanceLogs, isLoading, createFusedModel, createStrategy, deleteFusedModel, updateFusedModel };
}
