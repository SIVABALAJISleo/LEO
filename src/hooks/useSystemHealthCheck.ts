...
export function useSystemHealthCheck(autoRun = false, intervalMs = 300000) {
  ...
  const checkPricingConsistency = useCallback(async (): Promise<HealthCheckResult> => {
    ...
    if (!proPlan || proPlan.price !== EXPECTED_PRICING.pro.min || proPlan.priceMax !== EXPECTED_PRICING.pro.max) {
      issues.push(`PRO pricing mismatch: expected $${EXPECTED_PRICING.pro.min}-$${EXPECTED_PRICING.pro.max}`);
    }

    if (!heavyPlan || heavyPlan.price !== EXPECTED_PRICING.heavy.min || heavyPlan.priceMax !== EXPECTED_PRICING.heavy.max) {
      issues.push(`HEAVY pricing mismatch: expected $${EXPECTED_PRICING.heavy.min}-$${EXPECTED_PRICING.heavy.max}`);
    }
