...
export function useObservabilityData() {
  ...
  const fetchAll = async () => {
    ...
    const [rawRes, aggRes, tracesRes, anomaliesRes, corrRes] = await Promise.all([
      supabase.from('metrics_raw').select('*').order('recorded_at', { ascending: false }).limit(100),
      supabase.from('metrics_aggregated').select('*').order('created_at', { ascending: false }).limit(50),
  ...
  const resolveAnomaly = async (id: string) => {
    const { error } = await supabase.from('anomalies').update({ is_resolved: true, resolved_at: new Date().toISOString() }).eq('id', id);
    if (error) toast.error('Failed to resolve');
    else { toast.success('Resolved'); fetchAll(); }
  };
  ...
  return { metricsRaw, metricsAggregated, traces, anomalies, correlations, isLoading, recordMetric, resolveAnomaly, getAnomalyStats };
}
