...
export function MasterReductionPipeline() {
  ...
  const PIPELINE_STEPS: PipelineStep[] = [
    ...
    },
    {
      ...
      activeDescription: "Extracting downstream perceptual and numerical requirements (epsilon, SSIM, target FPS).",
      mathematicalAction: "Define Contract C = { ||Y - Y*|| / ||Y*|| <= eps, SSIM(I) >= 0.95, Latency <= 33ms }",
      hyperDecision: `Sets error budget eps = ${selectedContractTolerance} and downstream SLA.`,
      gpuBruteForceAction: "Ignores contract; mandates IEEE-754 FP32 bit-level execution regardless of user need.",
    },
    {
      id: 3,
      ...
      mathematicalAction: "Rank estimation: r = argmin_k (sigma_{k+1} / sigma_1 < eps), Sparsity S = count(W == 0) / N^2",
    ...
    },
    {
      ...
      activeDescription: "Scanning for repeated token prefixes, static BVH bounding boxes, or cached query clusters.",
      mathematicalAction: "Cosine similarity sim(q, q_cached) > 0.75, or static geometry cache hit.",
    ...
    },
    {
      ...
      activeDescription: "Substituting brute-force algorithms with sublinear, addition-only, or quasi-random solvers.",
      mathematicalAction: "Replace O(N^3) GEMM with O(Nkr) Randomized SVD + BitNet Ternary LUT additions.",
    ...
    },
    {
      ...
      activeDescription: "Dispatching kernels across 4 P-cores, 4 E-cores, and OpenVINO Intel UHD GPU.",
      mathematicalAction: "Partition load: 60% AVX2 vectorized CPU threads + 40% Intel UHD 48 EUs OpenVINO.",
    ...
    },
    {
      ...
      activeDescription: "Verifying output against quality contract via randomized Monte Carlo checks in O(N^2).",
      mathematicalAction: "Freivalds test: ||A * (B * r) - C_approx * r|| < eps * ||r|| with error prob < 2^{-k}.",
    ...
    },
    {
      ...
      activeDescription: "Delivering final verified result with 100% application parity and 0W idle power.",
      mathematicalAction: "Return Y_verified. If error > eps, seamlessly fallback to exact SIMD baseline.",
      ...
      <div className="rounded-xl border border-cyan-500/30 bg-black/80 p-6 backdrop-blur space-y-4">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-muted-foreground text-xs font-sans mt-1">
              How HYPER dynamically finds the cheapest computation that satisfies the required quality contract.
            </p>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            >
              {isGpuComparisonMode ? "Comparing vs GPU Brute Force" : "Show GPU Brute Force Contrast"}
            </button>
            ...
            >
              {isAutoPlaying ? <RotateCw className="h-3.5 w-3.5 animate-spin" /> : <Play className="h-3.5 w-3.5" />}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-3 border-t border-border/40">
          <div className="space-y-1.5">
            <label className="text-muted-foreground uppercase font-bold text-[10px]">Select Input Workload:</label>
            <select
            ...
            >
              ...
              <option value="nbody_physics">N-Body Astrodynamics Particle Cluster (512 particles)</option>
          ...
          <div className="space-y-1.5">
            <div className="flex justify-between text-[10px]">
              <label className="text-muted-foreground uppercase font-bold">Contract Quality Tolerance (ε):</label>
              <span className="font-bold text-cyan-400">ε = {selectedContractTolerance} ({(selectedContractTolerance * 100).toFixed(1)}% error budget)</span>
            </div>
            <input
              type="range"
              min={0.001}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-4">
            <div className="rounded-lg border border-border/60 bg-zinc-950 p-4 space-y-2">
              <span className="text-muted-foreground font-bold uppercase text-[10px]">Mathematical Transformation:</span>
            ...
            <div className="rounded-lg border border-cyan-500/30 bg-cyan-950/20 p-4 space-y-2">
              <span className="text-cyan-400 font-bold uppercase text-[10px]">HYPER Algorithmic Catalyst:</span>
          ...
          <div className="space-y-4">
            <div className="rounded-lg border border-border/60 bg-zinc-950 p-4 space-y-2">
              <span className="text-muted-foreground font-bold uppercase text-[10px]">Stage Description:</span>
              <p className="text-foreground text-xs leading-relaxed">{currentStep.activeDescription}</p>
            </div>

            {isGpuComparisonMode && (
              <div className="rounded-lg border border-red-500/30 bg-red-950/20 p-4 space-y-2">
                <span className="text-red-400 font-bold uppercase text-[10px]">NVIDIA GPU Brute-Force Path:</span>
                <p className="text-red-200/90 text-xs leading-relaxed">{currentStep.gpuBruteForceAction}</p>
              </div>
            )}
          </div>
        </div>
