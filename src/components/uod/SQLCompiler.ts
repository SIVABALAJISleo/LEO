...
export class SQLCompiler {
  ...
  private static buildQuery(node: Node<UOD_NodeData>, nodes: Node<UOD_NodeData>[], edges: Edge[]): string {
    ...
    switch (node.data.type) {
      ...
      case 'aggregate': {
        ...
        const selectCols = [...groupBy, ...aggs.map((a: any) => `${a.fn}(${a.col}) AS ${a.alias}`)].join(', ');
