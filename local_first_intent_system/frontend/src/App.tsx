...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (currentType && isPaletteOpen) {
      const prevId = selectedBlocks.length > 0 ? selectedBlocks[selectedBlocks.length - 1].id : undefined;
      ...
      <div className="relative max-w-5xl mx-auto pt-16 px-8">
        ...
        <div className="mb-12">
          ...
          <p className="text-xs text-zinc-500 max-w-xl leading-relaxed">
            Local-first execution system. Zero natural language ambiguity. 
            Meaning is aligned at the boundary through forced structural reification.
          </p>
        </div>
            ...
            <AnimatePresence>
              {selectedBlocks.map((block, i) => (
                ...
                >
                  ...
                  <button onClick={() => removeBlock(i)} className="text-zinc-700 hover:text-white ml-1">
                    <X size={12} />
                  </button>
                </motion.div>
              ))}
            ...
            {selectedBlocks.length < 4 && (
              <div className="flex-grow relative">
                <input
                  ...
                  placeholder={selectedBlocks.length === 0 ? "Identify System (Search...)" : "Align Next Intent Block..."}
                />
                  ...
                  {isPaletteOpen && suggestions.length > 0 && (
                    ...
                    >
                       ...
                       {suggestions.map((node) => (
                         ...
                         >
                           <span className="text-xs group-hover:text-brand transition-colors tracking-tight">{node.name}</span>
                           <ChevronRight size={12} className="text-zinc-800 group-hover:text-brand" />
                         </button>
                       ))}
                    </motion.div>
                  )}
            ...
            >
              <div className="col-span-2 bg-[#0c0c0e] border border-zinc-800 rounded-lg p-8">
                 <div className="flex items-center gap-3 text-brand mb-6">
                    <Zap size={18} />
                    <span className="text-xs font-bold uppercase tracking-widest">Execution Result // Resolved</span>
                 </div>
              ...
              <div className="bg-[#0c0c0e] border border-zinc-800 rounded-lg p-6">
                 <h4 className="text-[9px] uppercase tracking-widest text-zinc-600 mb-6">Local_Stats</h4>
          ...
          {error && (
            ...
            >
               ...
               <div className="text-xs opacity-70 leading-relaxed font-mono">
                 {error}: The requested intent sequence violated the deterministic graph schema. No reasoning recovery permitted.
               </div>
            </motion.div>
          )}
        </AnimatePresence>
