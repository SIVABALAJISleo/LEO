...
server.listen(0, '127.0.0.1', async () => {
  ...
  async function test(label, size, maxBodyLength, maxRedirects, expectBlocked) {
    ...
    try {
      ...
      if (expectBlocked) {
        console.log(`  [FAILED] ${label.padEnd(50)} -> Completed! (Received ${receivedBytes} bytes). VULNERABLE.`);
        failed++;
      } else {
  ...
  try {
    ...
    await test("Stream 100KB with maxBodyLength=10KB, maxRedirects=0", 100 * 1024, 10 * 1024, 0, true);
    ...
    await test("Stream 100KB with maxBodyLength=10KB, default redirects", 100 * 1024, 10 * 1024, undefined, true);

    console.log("\n======================================================");
    const total = passed + failed;
    if (failed === 0) {
      console.log(`  AUDIT PASSED: ${passed}/${total} checks secure. maxBodyLength bypass mitigated.`);
      process.exit(0);
    } else {
