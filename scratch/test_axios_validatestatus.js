...
function runTest(testName, config1, config2, expectDefaultCheck) {
  ...
  if (expectDefaultCheck) {
    ...
    if (accepts200 && !accepts401 && !accepts500) {
      ...
      console.log(
        `  [FAILED] ${testName.padEnd(50)} -> Default behavior compromised! (200:${accepts200}, 401:${accepts401}, 500:${accepts500})`,
      );
      failed++;
    }
  } else {
...
try {
  ...
  runTest(
    "Default validateStatus merge",
    { validateStatus: axios.defaults.validateStatus },
    {},
    true,
  );
  runTest(
    "Custom validateStatus override",
    { validateStatus: axios.defaults.validateStatus },
    { validateStatus: () => true },
    false,
  );
  ...
  runTest(
    "Default merge under active prototype pollution",
    { validateStatus: axios.defaults.validateStatus },
    {},
    true,
  );
  ...
  if (failed === 0) {
    console.log(
      `  AUDIT PASSED: ${passed}/${total} checks secure. validateStatus PP bypass mitigated.`,
    );
    process.exit(0);
  } else {
