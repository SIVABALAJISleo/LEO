...
server.listen(0, "127.0.0.1", async () => {
  ...
  try {
    ...
    if (hasRawCRLFInFilename) {
      console.log(
        "  [FAILED] CRLF injection into Content-Disposition filename succeeded! VULNERABLE.",
      );
      failed++;
    ...
    if (failed === 0) {
      console.log(
        `  AUDIT PASSED: ${passed}/${total} checks secure. CRLF multipart injection mitigated.`,
      );
      process.exit(0);
    } else {
