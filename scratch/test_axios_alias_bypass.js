...
function runTest(testName, noProxyValue, requestUrl, shouldBypass) {
  ...
  if (bypassed === shouldBypass) {
    ...
    console.log(`  [FAILED] ${testName.padEnd(55)} -> VULNERABLE! (Expected bypass: ${shouldBypass}, got: ${bypassed})`);
    failed++;
  }
}
...
runTest("Request to http://127.0.0.1/ (IPv4 loopback alias)", "localhost", "http://127.0.0.1/", true);
...
runTest("Request to internal non-loopback (should proxy)", "localhost", "http://192.168.1.5/", false);
