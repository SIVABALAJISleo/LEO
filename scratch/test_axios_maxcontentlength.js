...
server.listen(0, '127.0.0.1', async () => {
  ...
  async function test(label, maxContentLength, expectBlocked) {
    try {
      ...
      if (expectBlocked) {
        console.log(`  [FAILED] ${label.padEnd(50)} -> Read fully (${receivedBytes} bytes). VULNERABLE.`);
        failed++;
      } else {
  ...
  try {
    ...
    if (failed === 0) {
      console.log(`  AUDIT PASSED: ${passed}/${total} checks secure. maxContentLength stream bypass mitigated.`);
      process.exit(0);
    } else {
