...
class LinearAttention(nn.Module):
    ...
    def _feature_map(self, x: torch.Tensor) -> torch.Tensor:
        # Positive kernel feature map: ELU(x) + 1.0 (Katharopoulos et al.)
        return F.elu(x) + 1.0
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Compute (K^T V) first: (batch, heads, head_dim, head_dim) -> O(N * d^2)
        kv = torch.matmul(k_phi.transpose(-2, -1), v)
        
        # Multiply Q with KV: (batch, heads, seq_len, head_dim) -> O(N * d^2)
        out = torch.matmul(q_phi, kv)
        
        # Normalize by sum of K features
        k_sum = k_phi.sum(dim=-2, keepdim=True) # (batch, heads, 1, head_dim)
