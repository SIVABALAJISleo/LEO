...
const artifactDir = 'C:\\Users\\sivab\\.gemini\\antigravity\\brain\\5d53dfb3-5629-49dc-8e7c-31d35640d0f8';
...
async function runPhase(browser, phaseNum, phaseName, viewport, initScript) {
    ...
    } catch(e) { console.log(`  ✗ Extreme: ${e.message.split('\n')[0]}`); }
    ...
    } catch(e) { console.log(`  ✗ Start Test: ${e.message.split('\n')[0]}`); }
    ...
    const result = await page.evaluate(() => {
        ...
        if (!fpsNum) {
            for (const el of document.querySelectorAll('div,span')) {
                ...
                if (m) { fpsNum = parseInt(m[1]); break; }
            }
        }
        ...
        if (canvas) {
            ...
            if (gl) glInfo = {
                drawW: gl.drawingBufferWidth, drawH: gl.drawingBufferHeight,
...
async function main() {
    ...
    ]) { if (fs.existsSync(b)) { executablePath = b; break; } }
    ...
    const results = [];
    try {
        results.push(await runPhase(browser, 1,
            'BASELINE (1280x720, D3D11 only)',
            { width: 1280, height: 720 }, VISIBILITY_OVERRIDE));

        results.push(await runPhase(browser, 2,
            'RESOLUTION ONLY (640x360 viewport)',
            { width: 640, height: 360 }, VISIBILITY_OVERRIDE));

        results.push(await runPhase(browser, 3,
            'SHADER ALCHEMY (1280x720 + GLSL hook)',
            { width: 1280, height: 720 }, VISIBILITY_OVERRIDE + SHADER_HOOK));

        results.push(await runPhase(browser, 4,
            'FULL CONVERGENCE (640x360 + GLSL hook)',
            { width: 640, height: 360 }, VISIBILITY_OVERRIDE + SHADER_HOOK));
    ...
    for (const r of results) {
        ...
        console.log(`  ${r.phaseName.substring(0,40).padEnd(40)} ${fps.padEnd(6)} ${buf.padEnd(12)} ${css}`);
    }
    console.log('═'.repeat(72));
}
