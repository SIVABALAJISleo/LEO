...
export const INVALIDATION_TYPES = [
  ...
  { value: 'lru_eviction', label: 'LRU Eviction', description: 'Evicted due to cache full (least recently used)' },
];
...
export const useCachingData = () => {
  ...
  const recordAnalytics = async (cacheLevel: string, hits: number, misses: number, evictions: number = 0) => {
    if (!user) return;
  ...
  const getEstimatedSpeedup = () => {
    ...
    const avgTimeSaved = recentAnalytics.reduce((sum, a) => sum + (a.avg_response_time_saved_ms || 0), 0) / recentAnalytics.length;
