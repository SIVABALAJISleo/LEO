...
export const useMultiCloudData = () => {
  ...
  const deleteProvider = async (providerId: string) => {
    ...
    try {
      const { error } = await supabase.from("cloud_providers").delete().eq("id", providerId);
  ...
  const createRoutingRule = async (data: { name: string; mode: string; conditions?: Json }) => {
    if (!user) return null;
    ...
    switch (mode) {
      ...
      case "lowest_cost":
        return activeProviders.reduce((best, current) => {
          ...
          return currentStats.latestCost.cost_per_request < bestStats.latestCost.cost_per_request
            ? current
            : best;
        });
