...
export class EngineeringCeilingScore {
  calculateScore(
    ...
    const reasoningQualityPct = Math.max(50, parseFloat((100 - (contradictionsCount * 15)).toFixed(1)));
    ...
    const roboticsReasoningPct = parseFloat((sensorConfidence * 100 * (1 - collisionRisk / 100)).toFixed(1));
    
    // Scientific is affected by float errors
    const scientificAssistancePct = Math.max(50, parseFloat((100 - (precisionErrorWorstCase * 100)).toFixed(1)));
