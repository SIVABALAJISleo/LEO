...
export class UserSatisfactionEngine {
  calculateUtility(tele: UserSatisfactionTelemetry): {
    satisfactionIndex: number;
    realWorldUtilityScore: number;
  } {
    ...
    const satisfactionIndex = parseFloat(
      (
        (resolutionSuccessFactor * 0.3 +
          workflowCompletionFactor * 0.3 +
          lowRetryFactor * 0.2 +
          positiveFeedbackFactor * 0.2) *
        100
      ).toFixed(1),
    );
