...
export class RealUserFeedbackLearningEngine {
  ...
  generateProfile(): {
    ...
    confidenceAdjustedLearningWeights: Record<string, number>;
  } {
    ...
    const workflowCompletionScore = parseFloat(
      (((acceptedCount + editedCount) / total) * 100).toFixed(1),
    );
    ...
    const confidenceAdjustedLearningWeights = {
      ...
      verificationRigidityWeight: parseFloat((0.6 + (correctionScore / 100) * 0.25).toFixed(3)),
      prefixMatchConfidence: parseFloat((0.9 - (clarificationScore / 100) * 0.2).toFixed(3)),
    };
