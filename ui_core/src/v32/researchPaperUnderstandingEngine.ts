...
export class ResearchPaperUnderstandingEngine {
  analyzePaper(rawMarkdown: string): ClinicalStudy {
    ...
    let methodology =
      "Double-blind randomized controlled trial, sample N=240, running local LLM configurations.";
    ...
    if (text.includes("accuracy") || text.includes("qlora")) {
      methodology =
        "Comparative benchmark analysis of QLoRA INT4 parameters vs standard FP16 baseline.";
