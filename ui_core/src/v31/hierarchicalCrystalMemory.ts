...
export type CrystalMemoryLevel =
  | "L0_Instant_Cache"
  | "L1_Semantic_Cache"
  | "L2_Graph_Cache"
  | "L3_Memory_Store"
...
export class HierarchicalCrystalMemory {
  ...
  private semanticEmbeddings: { text: string; synonyms: string[]; answer: string }[] = [
    {
      ...
      answer:
        "Utilize Phase 1 speculative decoding engine along with Phase 2 INT4 AWQ quantization profiles.",
    },
    {
      text: "what is paged memory",
      synonyms: ["vllm attention", "elastic gpu vram", "paged cache compaction"],
      answer:
        "Paged memory uses dynamic block indexing mappings to distribute KV items flexibly, preventing fragmentation.",
    },
  ];
  ...
  routeQuery(query: string): RetrievalAudit {
    ...
    if (l0Hit) {
      ...
      const semanticMatch = this.semanticEmbeddings.find(
        (item) =>
