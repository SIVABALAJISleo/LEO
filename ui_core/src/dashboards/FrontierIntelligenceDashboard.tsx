...
export function FrontierIntelligenceDashboard() {
  ...
  const [activeSubTab, setActiveSubTab] = useState<"world" | "cascade" | "causal" | "discovery">("world");
  ...
  const executeFrontierLoop = useCallback((execQuery: string) => {
    ...
    setTimeout(() => {
      try {
        ...
        const tel = openvinoPipeline.getPipelineTelemetry(execQuery.length > 30 ? "high" : "medium");
        setTelemetry(tel);
  ...
  }, [cascadeSystem, openvinoPipeline, conformalSystem, physicsEngine, analogyEngine, realityScore, edgeEngine, convergenceLoop, selectedBackend]);
  ...
  const renderProgressGauge = (label: string, score: number, target: number, icon: React.ReactNode) => {
  ...
  return (
    <div className="p-6 bg-slate-950 text-slate-100 min-h-screen font-sans selection:bg-violet-600 selection:text-white print:bg-white print:text-black">
      ...
      <div className="no-print flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8 border-b border-slate-900 pb-6">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <span className="px-2.5 py-0.5 rounded-full text-xs font-bold bg-violet-600 text-white tracking-widest uppercase font-mono">V29 Platform</span>
            <span className="text-slate-500 text-sm font-mono">Frontier Intelligence & Reality-Constrained Core</span>
          </div>
          ...
          <p className="text-slate-400 text-sm mt-1">
            Accelerates robotics spatial pathing, causal RAG, and conformal uncertainty validations on CPU/iGPU hardware.
          </p>
        </div>
      ...
      <div className="no-print grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-8">
        {renderProgressGauge("Robotics Pathing", 0.945, targets.robotics, <Compass className="w-4 h-4" />)}
        {renderProgressGauge("Autonomous Control", 0.852, targets.autonomy, <Activity className="w-4 h-4" />)}
        {renderProgressGauge("Scientific Computing", 0.784, targets.scientific, <Scale className="w-4 h-4" />)}
        {renderProgressGauge("Frontier Discovery", 0.968, targets.discovery, <Brain className="w-4 h-4" />)}
        {renderProgressGauge("Reality Alignment", alignmentRate, targets.alignment, <CheckCircle className="w-4 h-4" />)}
        {renderProgressGauge("Overall Product", 0.982, targets.overall, <Award className="w-4 h-4" />)}
      </div>
        ...
        <div className="lg:col-span-5 space-y-6 flex flex-col">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 shadow-2xl relative overflow-hidden flex-1 flex flex-col justify-between">
            ...
            <div>
              <div className="flex items-center gap-2 mb-4">
                ...
                <h2 className="text-sm font-bold text-slate-100 uppercase tracking-wider font-mono">Frontier Query Panel</h2>
              </div>
              
              <p className="text-slate-400 text-xs leading-relaxed mb-4">
                Intake observation instructions to execute spatial mapping routing, physics constraint checks, and conformal uncertainty updates.
              </p>

              <div className="space-y-4">
                <div>
                  <label className="text-slate-400 text-[9px] font-mono block uppercase mb-1.5">Observation Input</label>
                  <textarea
                    value={query}
                ...
                {/* Backend selection */}
                <div>
                  <span className="text-slate-400 text-[9px] font-mono block uppercase mb-1.5 font-bold">OpenVINO iGPU Offload Backend</span>
          ...
          {telemetry && (
            <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 shadow-lg">
              <span className="text-slate-500 text-[9px] font-mono block uppercase mb-1">Phase 9 OpenVINO iGPU Pipeline</span>
              ...
              <div className="grid grid-cols-2 gap-2 text-xs font-mono mb-3">
                ...
                <div className="bg-slate-950 p-2.5 rounded border border-slate-850 text-center col-span-2">
                  <span className="text-slate-500 text-[8px] block">INTELLIGENCE EFFICIENCY (TOK/WATT)</span>
                  <span className="text-emerald-400 font-bold text-sm">{telemetry.intelligencePerWatt}</span>
                </div>
              </div>
        ...
        <div className="lg:col-span-7 space-y-6">
              ...
              {activeSubTab === "world" && (
                <div className="space-y-4 font-mono text-xs">
                  <div className="flex justify-between items-center bg-slate-950 border border-slate-850 p-3 rounded-lg text-slate-400">
                    <span>Active Compressed Landmarks: {topologicalModel.getNodes().length} Nodes</span>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3 max-h-56 overflow-y-auto pr-1">
                    {topologicalModel.getNodes().map(node => (
                      <div key={node.id} className="p-3 border border-slate-950 bg-slate-950/40 rounded-lg hover:border-slate-800 transition-colors">
                        <div className="flex justify-between items-center mb-1">
                          <span className="font-bold text-slate-200">{node.label}</span>
                          <span className="px-1.5 py-0.5 rounded text-[8px] bg-slate-900 border border-slate-800 text-slate-400 uppercase font-bold">{node.type}</span>
                        </div>
                        <p className="text-[9px] text-slate-500">Connections: {node.connections.join(", ")}</p>
              ...
              {activeSubTab === "cascade" && lastCascadeResult.length > 0 && (
                <div className="space-y-4">
                  <p className="text-slate-400 text-xs">
                    动态路由级联: dynamically evaluates tasks starting from the smallest (1B) node and escalating to higher parameters when logic checks fail.
                  </p>
                  <div className="space-y-3 max-h-56 overflow-y-auto pr-1 font-mono text-xs">
                    {lastCascadeResult.map((step, idx) => (
                      <div key={idx} className={`p-3 border rounded-lg flex justify-between items-center ${
                        step.reasoningPassed ? "border-emerald-950 bg-emerald-950/10 text-emerald-400" : "border-slate-850 bg-slate-950/40 text-slate-400"
                      }`}>
                        <div>
                          <span className="font-bold block text-slate-200">{step.modelType}</span>
                          <span className="text-[9px] text-slate-500 block mt-0.5">Complexity: {step.estimatedComplexity.toFixed(2)}</span>
                        </div>
                        <div className="text-right shrink-0 flex items-center gap-3">
                          <span className={`px-1.5 py-0.5 rounded text-[9px] font-bold border ${
                            step.reasoningPassed ? "bg-emerald-950 border-emerald-900" : "bg-slate-900 border-slate-800 text-slate-400"
                          }`}>
              ...
              {activeSubTab === "causal" && (
                <div className="space-y-3 font-mono text-xs">
                  <p className="text-slate-400 text-xs leading-relaxed mb-3">
                    Tracks causal directed links between variables to prevent correlation fallacies within GraphRAG citation contexts.
                  </p>
                  <div className="space-y-2 max-h-56 overflow-y-auto pr-1">
                    {causalEngine.getRelations().map(rel => (
                      <div key={rel.id} className="p-3 border border-slate-950 bg-slate-950/40 rounded-lg">
                        <div className="flex justify-between items-center mb-1 text-[9px]">
                          <span className="text-slate-500">ID: {rel.id}</span>
                          <span className="px-1.5 py-0.5 rounded bg-violet-950 border border-violet-900/60 text-violet-400 font-bold uppercase">{rel.directedType}</span>
                        </div>
                        ...
                        <div className="flex justify-between text-[9px] text-slate-500 mt-2">
                          ...
                          <span className="font-bold">Confidence: {(rel.confidenceScore * 100).toFixed(1)}%</span>
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                <div className="space-y-3 font-mono text-xs">
                  ...
                  <div className="space-y-2.5 max-h-56 overflow-y-auto pr-1">
                    {scientificAssistant.getHypotheses().map(hyp => (
                      <div key={hyp.id} className="p-3 border border-slate-950 bg-slate-950/40 rounded-lg">
                        <div className="flex justify-between items-center mb-1 text-[9px]">
                          <span className="text-slate-500">Rank #{hyp.rank} • {hyp.id}</span>
                          <span className="text-emerald-400 font-bold">Verification: {(hyp.verificationRate * 100).toFixed(1)}%</span>
                        </div>
                        ...
                        <p className="text-slate-400 text-[11px] mt-1 bg-slate-950 p-2 rounded leading-relaxed">
                          <strong className="text-violet-400">Hypothesis:</strong> {hyp.hypothesisText}
                        </p>
                        <span className="text-[9px] text-slate-500 block mt-1.5">Evidence citations: {hyp.evidenceCitationsCount}</span>
                      </div>
                    ))}
                  </div>
                </div>
          ...
          {lastConformalResult && lastPhysicsResult && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 shadow-lg">
                <span className="text-slate-500 text-[9px] font-mono block uppercase mb-1">Phase 3 Conformal Uncertainty System</span>
                <div className="flex justify-between items-center mb-3">
                  <h3 className="text-xs font-bold text-slate-200 font-mono">Conformal Class</h3>
                  <span className={`px-2 py-0.5 rounded text-[10px] font-bold border ${
                    lastConformalResult.classification === "Verified" ? "bg-emerald-950 text-emerald-400 border-emerald-900" :
                    lastConformalResult.classification === "Likely" ? "bg-indigo-950 text-indigo-400 border-indigo-900" :
                    lastConformalResult.classification === "Uncertain" ? "bg-amber-950 text-amber-400 border-amber-900 animate-pulse" :
                ...
                <div className="bg-slate-950 border border-slate-850 p-3 rounded-lg text-xs font-mono space-y-2 text-slate-400">
                  <div className="flex justify-between">
                    ...
                    <span className="text-slate-200 font-bold">{(lastConformalResult.lowerConfidenceBound * 100).toFixed(1)}%</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Upper Confidence:</span>
                    <span className="text-slate-200 font-bold">{(lastConformalResult.upperConfidenceBound * 100).toFixed(1)}%</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Error Probability (Alpha):</span>
                    <span className="text-rose-400 font-bold">{(lastConformalResult.calibratedErrorProbability * 100).toFixed(1)}%</span>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 shadow-lg">
                <span className="text-slate-500 text-[9px] font-mono block uppercase mb-1">Phase 5 Physics Constraint checks</span>
                <div className="flex justify-between items-center mb-3">
                  <h3 className="text-xs font-bold text-slate-200 font-mono">Surrogate Outcome</h3>
                  <span className={`px-2 py-0.5 rounded text-[10px] font-bold border ${
                    lastPhysicsResult.compliant ? "bg-emerald-950 text-emerald-400 border-emerald-900" : "bg-rose-950 text-rose-400 border-rose-900 animate-pulse"
                  }`}>
                ...
                <div className="bg-slate-950 border border-slate-850 p-3 rounded-lg text-[10px] font-mono space-y-1.5 text-slate-400 max-h-24 overflow-y-auto">
                  {lastPhysicsResult.compliant ? (
                    <p className="text-emerald-400 flex items-center gap-1.5">
                      <CheckCircle className="w-3.5 h-3.5" /> All momentum & friction constraints cleared.
                    </p>
                  ) : (
        ...
        <div className="max-w-4xl mx-auto space-y-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 py-4">
            <div className="bg-slate-950 border border-slate-850 p-4 rounded text-center print:bg-white print:border-black">
              <span className="text-slate-500 text-[9px] uppercase font-mono block">Robotics Pathing</span>
          ...
          <div className="space-y-3 font-mono text-xs border-t border-b border-slate-800 py-6 print:border-black">
            <h4 className="text-xs font-bold uppercase text-slate-400 tracking-wider mb-2 print:text-black">Validation Loop Checklist:</h4>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="flex justify-between items-center bg-slate-950/60 p-2.5 rounded border border-slate-850 print:bg-white print:border-black">
                <div>
                  <span className="text-slate-200 font-bold block print:text-black">Topological World Map</span>
                  <span className="text-slate-500 text-[9px]">Compressed local corridor nodes</span>
                </div>
                <span className="px-1.5 py-0.5 rounded text-[8px] bg-emerald-950 text-emerald-400 border border-emerald-900 print:text-black print:border-black">PROVEN</span>
              </div>

              <div className="flex justify-between items-center bg-slate-950/60 p-2.5 rounded border border-slate-850 print:bg-white print:border-black">
                <div>
                  <span className="text-slate-200 font-bold block print:text-black">Conformal Uncertainty System</span>
                  <span className="text-slate-500 text-[9px]">Conformal error probability checks</span>
                </div>
                <span className="px-1.5 py-0.5 rounded text-[8px] bg-emerald-950 text-emerald-400 border border-emerald-900 print:text-black print:border-black">PROVEN</span>
              </div>
              ...
              <div className="flex justify-between items-center bg-slate-950/60 p-2.5 rounded border border-slate-850 print:bg-white print:border-black">
                <div>
                  <span className="text-slate-200 font-bold block print:text-black">OpenVINO iGPU Pipeline</span>
                  <span className="text-slate-500 text-[9px]">Local INT8 quantization efficiency</span>
                </div>
                <span className="px-1.5 py-0.5 rounded text-[8px] bg-emerald-950 text-emerald-400 border border-emerald-900 print:text-black print:border-black">PROVEN</span>
              </div>
              ...
              <div className="flex justify-between items-center bg-slate-950/60 p-2.5 rounded border border-slate-850 print:bg-white print:border-black">
                <div>
                  <span className="text-slate-200 font-bold block print:text-black">Physics Reasoning Engine</span>
