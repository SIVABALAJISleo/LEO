...
import {
  ...
  MasterReport,
} from "../failure_hunting/calculators/masterReportGenerator";
import {
  ShieldAlert,
  Crosshair,
  AlertTriangle,
  ...
  if (loading || !report) {
    return (
      <div className="flex flex-col items-center justify-center h-full text-red-500 animate-pulse bg-black/90 p-10">
        ...
        <h2 className="text-2xl font-bold uppercase tracking-widest text-red-600">
          Hunting Weaknesses...
        </h2>
        <p className="text-red-400/70 text-sm mt-2 font-mono">
          Running Adversarial Failure Injections
        </p>
      </div>
    );
  }
        ...
        <section>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {Object.entries(report.scores).map(([key, value]) => (
              <div
                key={key}
                className="bg-red-950/20 border border-red-900/30 p-4 rounded print:border-gray-300 print:bg-white"
              >
          ...
          <section>
            ...
            <ul className="space-y-3">
              {report.topWeaknesses.map((weakness, i) => (
                <li
                  key={i}
                  className="bg-orange-950/20 border-l-4 border-orange-600 p-3 text-sm text-orange-200 print:text-black print:border-gray-800"
                >
                  {weakness}
                </li>
              ))}
          ...
          <section>
            ...
            <ul className="space-y-3">
              {report.topBottlenecks.map((bottleneck, i) => (
                <li
                  key={i}
                  className="bg-red-950/20 border-l-4 border-red-600 p-3 text-sm text-red-200 print:text-black print:border-gray-800"
                >
                  {bottleneck}
                </li>
              ))}
        ...
        <div className="grid md:grid-cols-2 gap-8 print-break">
          ...
          <section>
            ...
            <ul className="space-y-3">
              {report.limitingFactors.map((factor, i) => (
                <li
                  key={i}
                  className="bg-yellow-950/20 border-l-4 border-yellow-600 p-3 text-sm text-yellow-200 print:text-black print:border-gray-800"
                >
                  {factor}
                </li>
              ))}
          ...
          <section>
            ...
            <div className="bg-green-950/20 border border-green-900/50 p-6 rounded mb-6 print:border-gray-400">
              <div className="text-green-400 text-sm uppercase mb-2 print:text-gray-600">
                Estimated Ceiling After Fixes
              </div>
              <div className="text-5xl font-black text-green-400 print:text-black">
                {report.estimatedCeilingAfterFixes}%
              </div>
            </div>
