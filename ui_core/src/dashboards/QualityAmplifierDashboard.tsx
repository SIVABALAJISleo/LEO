...
import { V22Orchestrator, AmplificationCycleResult, QualityScores } from "../v22/v22index";
import {
  Zap,
  Brain,
  ShieldCheck,
...
const ScoreCard = ({
  ...
  const good = isRate ? value < 0.01 : value >= 0.9;
  const border = isRate
    ? value < 0.01
  ...
  return (
    <div className={`bg-gray-900/80 border ${border} rounded-xl p-4 flex flex-col gap-1`}>
      ...
      <div
        className={`text-2xl font-black ${isRate ? (value < 0.01 ? "text-emerald-400" : "text-rose-400") : good ? "text-emerald-400" : "text-amber-400"}`}
      >
        {display}
      </div>
          ...
          <div>
            ...
            <p className="text-xs text-gray-400 max-w-xl">
              ...
              <span className="text-indigo-300 font-semibold">
                Measured Benefit &gt; Complexity Added
              </span>
            </p>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-6 py-8 space-y-6">
        ...
        <div className="bg-gray-900/60 border border-gray-700/50 rounded-xl p-5">
          ...
          {result && (
            <div className="mt-3 flex items-center gap-3 text-xs text-gray-500">
              ...
              <span
                className={
                  result.evalReport.releaseGate === "PASS"
                    ? "text-emerald-400 font-bold"
                    : result.evalReport.releaseGate === "FAIL"
        ...
        {!result && !loading && (
          <div className="text-center py-20 text-gray-600">
            ...
            <p className="text-lg font-bold">
              Run an amplification cycle to see live quality metrics
            </p>
        ...
        {loading && (
          <div className="text-center py-20">
            ...
            <p className="text-indigo-300 font-bold animate-pulse">
              Running 12-phase quality amplification...
            </p>
            <p className="text-xs text-gray-500 mt-1">
              Reasoning • Hallucination • Memory • Agents • Knowledge • Reality • Enterprise • Eval
              • Perf • Improvement
            </p>
          </div>
        )}
        ...
        {result && s && (
          <>
            ...
            >
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
                ...
                <ScoreCard
                  icon={Users}
                  label="Agent Quality"
                  value={s.agentQuality}
                  color="bg-amber-500"
                />
                <ScoreCard
                  icon={BookOpenCheck}
                  label="Knowledge"
                  value={s.knowledgeQuality}
                  color="bg-teal-500"
                />
                <ScoreCard
                  icon={Languages}
                  label="Language"
                  value={s.languageUnderstanding}
                  color="bg-cyan-500"
                />
                <ScoreCard
                  icon={ShieldCheck}
                  label="Enterprise Trust"
                  value={s.enterpriseTrust}
                  color="bg-green-500"
                />
                <ScoreCard
                  icon={Activity}
                  label="Overall Score"
                  value={s.overallProductScore}
                  color="bg-indigo-600"
                />
              </div>
            </SectionWrapper>
            ...
            <SectionWrapper
              id="failures"
              title="Phase 1 — Top-10 Failures Addressed"
              icon={Target}
              expanded={expandedSection === "failures"}
            ...
            >
              <div className="grid md:grid-cols-2 gap-2">
                {result.topFailuresAddressed.map((f, i) => (
                  <div
                    key={i}
                    className="flex items-start gap-2 bg-gray-950/60 border border-gray-800/60 rounded-lg p-3 text-xs"
                  >
                    <span className="text-indigo-400 font-black mt-0.5">#{i + 1}</span>
                    <div>
                      <div className="text-gray-200 font-semibold">{f}</div>
                      <div className="text-gray-500 mt-0.5">
                        Root cause analyzed → fix deployed → retested ✓
                      </div>
                    </div>
              ...
              <div className="mt-3 text-xs text-gray-500 border-t border-gray-800/50 pt-3">
                Current improvement:{" "}
                <span className="text-indigo-300 font-semibold">{result.improvementSummary}</span>
              </div>
            </SectionWrapper>
            ...
            <SectionWrapper
              id="agents"
              title="Phase 6 — Agent Performance Leaderboard"
              icon={Users}
                    ...
                    {result.agentLeaderboard.map((a, i) => (
                      ...
                      >
                        <td className="py-2 pr-4">
                          <span
                            className={`font-black ${i === 0 ? "text-amber-400" : i < 3 ? "text-gray-300" : "text-gray-600"}`}
                          >
                            {i + 1}
                          </span>
                        </td>
                        ...
                        <td className="py-2 pr-4 text-right text-violet-400">
                          {pct(a.reliabilityScore)}
                        </td>
                        <td className="py-2 text-right font-black text-indigo-300">
                          {pct(a.compositeScore)}
                        </td>
                      </tr>
                    ))}
                  </tbody>
            ...
            <SectionWrapper
              id="eval"
              title="Phase 10 — Evaluation at Scale"
              icon={BarChart3}
            ...
            >
              <div className="grid grid-cols-3 gap-3 mb-4">
                <div className="bg-gray-950 border border-gray-800 rounded-lg p-3 text-center">
                  ...
                  <div className="text-xl font-black text-indigo-400">
                    {result.evalReport.totalTasksSimulated.toLocaleString()}
                  </div>
                </div>
                <div className="bg-gray-950 border border-gray-800 rounded-lg p-3 text-center">
                  <div className="text-[9px] text-gray-500 uppercase tracking-widest">
                  ...
                  <div className="text-xl font-black text-emerald-400">
                    {pct(result.evalReport.overallAccuracy, 2)}
                  </div>
                </div>
                <div
                  className={`bg-gray-950 border rounded-lg p-3 text-center ${result.evalReport.releaseGate === "PASS" ? "border-emerald-500/40" : result.evalReport.releaseGate === "FAIL" ? "border-rose-500/40" : "border-amber-500/40"}`}
                >
                  <div className="text-[9px] text-gray-500 uppercase tracking-widest">
                    Release Gate
                  </div>
                  <div
                    className={`text-xl font-black ${result.evalReport.releaseGate === "PASS" ? "text-emerald-400" : result.evalReport.releaseGate === "FAIL" ? "text-rose-400" : "text-amber-400"}`}
                  >
              ...
              <div className="space-y-2">
                {result.evalReport.domainBenchmarks.map((d) => (
                  <div
                    key={d.domain}
                    className="flex items-center gap-3 bg-gray-950/60 border border-gray-800/50 rounded-lg px-3 py-2"
                  >
                    <span className="w-24 text-gray-300 font-semibold text-[11px] shrink-0">
                      {d.domain}
                    </span>
                    <div className="flex-1">
                      <div className="flex justify-between text-[10px] mb-1">
                        <span className="text-gray-500">
                          {d.tasksSimulated.toLocaleString()} tasks
                        </span>
                        <span
                          className={d.accuracy >= 0.92 ? "text-emerald-400" : "text-amber-400"}
                        >
                          {pct(d.accuracy, 2)}
                        </span>
                      </div>
                      <ScoreBar
                        value={d.accuracy}
                        color={d.accuracy >= 0.92 ? "bg-emerald-500" : "bg-amber-500"}
                      />
                    </div>
            ...
            <SectionWrapper
              id="perf"
              title="Phase 11 — Performance Governor"
              icon={Gauge}
              expanded={expandedSection === "perf"}
              ...
              <div className="grid md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <div className="mt-3 text-xs font-bold text-emerald-400">
                    Efficiency Gain: +{result.perfReport.efficiencyGainPct.toFixed(1)}%
                    intelligence/watt
                  </div>
                </div>
              </div>
            ...
            <SectionWrapper
              id="improve"
              title="Phase 12 — Autonomous Improvement Loop"
              icon={TrendingUp}
            ...
            >
              <div className="grid grid-cols-4 gap-3 mb-4">
                <MetaBadge
                  label="Total Cycles"
                  value={String(result.improvementState.totalCycles)}
                  color="text-indigo-400"
                />
                <MetaBadge
                  label="Current Score"
                  value={pct(result.improvementState.currentScore, 2)}
                  color="text-emerald-400"
                />
                <MetaBadge
                  label="Est. Ceiling"
                  value={pct(result.improvementState.estimatedCeiling, 1)}
                  color="text-violet-400"
                />
                <MetaBadge
                  label="Status"
                  value={result.improvementState.isConverging ? "Converging" : "Improving"}
                  color={
                    result.improvementState.isConverging ? "text-amber-400" : "text-emerald-400"
                  }
                />
              </div>
              <div className="space-y-2">
                {result.improvementState.recentCycles.map((c) => (
                  <div
                    key={c.cycleId}
                    className="bg-gray-950/60 border border-gray-800/50 rounded-lg p-3 text-xs"
                  >
                    ...
                    <div className="text-gray-400">
                      <span className="text-rose-400 font-semibold">Weakness: </span>
                      {c.weaknessFound}
                    </div>
                    <div className="text-gray-400 mt-0.5">
                      <span className="text-emerald-400 font-semibold">Fix: </span>
                      {c.improvementApplied}
                    </div>
  ...
  return (
    <div className="space-y-1">
      {rows.map((r) => (
        <div
          key={r.label}
          className="flex justify-between text-xs bg-gray-950/40 border border-gray-800/40 rounded px-2.5 py-1.5"
        >
          <span className="text-gray-500">{r.label}</span>
          <span className={highlight ? "text-emerald-400 font-bold" : "text-gray-300"}>
            {r.value}
          </span>
        </div>
      ))}
