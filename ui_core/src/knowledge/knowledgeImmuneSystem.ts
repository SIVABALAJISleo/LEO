...
export class KnowledgeImmuneSystem {
  private crystals: KnowledgeCrystal[] = [
    { id: "V16-C01", topic: "V16 Core Substrate Layout", trustScore: 0.98, freshnessScore: 0.95, accuracyScore: 0.99, evidenceScore: 0.96, confidenceScore: 0.98, verificationScore: 0.98, usageScore: 0.90, status: "active" },
    { id: "V16-C02", topic: "WebGPU Swarm Compiler pipelines", trustScore: 0.94, freshnessScore: 0.92, accuracyScore: 0.95, evidenceScore: 0.90, confidenceScore: 0.94, verificationScore: 0.92, usageScore: 0.85, status: "active" },
    { id: "V16-C03", topic: "WASM fallback compiler modes", trustScore: 0.45, freshnessScore: 0.30, accuracyScore: 0.50, evidenceScore: 0.35, confidenceScore: 0.42, verificationScore: 0.40, usageScore: 0.12, status: "active" },
    { id: "V16-C04", topic: "Malformed Stripe Signature Webhooks", trustScore: 0.15, freshnessScore: 0.85, accuracyScore: 0.10, evidenceScore: 0.05, confidenceScore: 0.18, verificationScore: 0.05, usageScore: 0.99, status: "active" },
    // Backward compatibility for V15 Tests
    { id: "V15-C01", topic: "V15 Core Substrate Layout", trustScore: 0.98, freshnessScore: 0.95, accuracyScore: 0.99, evidenceScore: 0.96, confidenceScore: 0.98, verificationScore: 0.98, usageScore: 0.90, status: "active" },
    { id: "V15-C04", topic: "Malformed Stripe Signature Webhooks", trustScore: 0.15, freshnessScore: 0.85, accuracyScore: 0.10, evidenceScore: 0.05, confidenceScore: 0.18, verificationScore: 0.05, usageScore: 0.99, status: "active" }
  ];
  ...
  public auditCrystals(): KnowledgeCrystal[] {
    this.crystals = this.crystals.map(crystal => {
      ...
      if (crystal.trustScore < 0.30 || crystal.verificationScore < 0.20) {
        return {
          ...crystal,
          status: "quarantined",
      ...
      if (crystal.freshnessScore < 0.50 && crystal.usageScore < 0.20) {
        return {
          ...crystal,
          status: "decayed",
      ...
      if (crystal.trustScore > 0.90 && crystal.verificationScore > 0.90 && crystal.usageScore > 0.70) {
        return {
          ...crystal,
          status: "strengthened",
          ...
          verificationScore: Math.min(0.99, parseFloat((crystal.verificationScore * 1.05).toFixed(4)))
        };
      }
