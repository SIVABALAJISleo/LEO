...
export class RealityFeedbackEngineV3 {
   ...
   */
  public logRealityEvent(predictionId: string, metricType: string, predictedVal: number, observedVal: number): FeedbackLog {
    const errorPct = predictedVal === 0 ? 0 : parseFloat((Math.abs(predictedVal - observedVal) / predictedVal * 100).toFixed(2));
    ...
    if (this.currentWeights[metricType] !== undefined) {
      ...
      this.currentWeights[metricType] = parseFloat(Math.max(0.5, Math.min(0.99, nextWeight)).toFixed(4));
    }
