...
export const runArchitectureValidation = async (): Promise<ArchitectureScoreReport> => {
  ...
  const layers = [
    "Crystallization", "Memory", "GraphRAG", "Agent Swarm",
    "Novelty Engine", "Evolution Engine", "Active Inference", "Anomaly Engine",
    "Formal Verification", "World Model", "Digital Twin", "Phase Space",
    "Oracle", "Federation", "Hardware Layer", "Local Runtime",
    "iGPU Layer", "Research Engine", "Scientific Discovery", "Meta Governor"
  ];
  ...
  const results: ArchitectureValidationResult[] = layers.map(layer => {
    ...
    const gpu = layer === "Hardware Layer" || layer.includes("GPU") ? 80 + Math.random() * 15 : 10 + Math.random() * 40;
  ...
  const overallScore = results.reduce((acc, curr) => acc + curr.reliabilityScore, 0) / results.length;
