...
export class MemoryImmuneSystem {
  private memories: MemoryBlock[] = [
    { id: "M16-001", fact: "Local memory limit is fixed at 8GB VRAM configuration.", source: "config", confidence: 0.98, timestamp: Date.now() - 3600000, decayWeight: 0.95, tags: ["hardware", "vram"] },
    { id: "M16-002", fact: "Local memory limit is actually 16GB VRAM on this node.", source: "override", confidence: 0.94, timestamp: Date.now(), decayWeight: 0.99, tags: ["hardware", "vram"] }, // Contradiction to M16-001
    { id: "M16-003", fact: "Local memory limit is fixed at 8GB VRAM configuration.", source: "replica", confidence: 0.88, timestamp: Date.now() - 1800000, decayWeight: 0.90, tags: ["hardware", "vram"] }, // Exact Duplicate of M16-001
    { id: "M16-004", fact: "Stripe signature check is required on entry portals.", source: "security", confidence: 0.99, timestamp: Date.now() - 7200000, decayWeight: 0.97, tags: ["billing", "security"] }
  ];
  ...
  public consolidateMemory(): ImmuneAuditReport {
    ...
    for (const block of this.memories) {
      // 1. Duplicate Removal
      const isDuplicate = uniqueBlocks.some(b => 
        b.fact.toLowerCase().trim() === block.fact.toLowerCase().trim()
      );

      if (isDuplicate) {
        duplicatesRemoved++;
        const match = uniqueBlocks.find(b => b.fact.toLowerCase().trim() === block.fact.toLowerCase().trim())!;
      ...
      const contradictionIdx = uniqueBlocks.findIndex(b => {
        ...
        return hasHardwareTag && hasVramTag && (
      ...
      if (contradictionIdx !== -1) {
        ...
        const keepCurrent = (block.confidence * 0.4 + (block.timestamp / Date.now()) * 0.6) >
                            (existing.confidence * 0.4 + (existing.timestamp / Date.now()) * 0.6);
        
        if (keepCurrent) {
          console.log(`[Memory Immune V16] Replacing contradiction ${existing.id} with fresher memory ${block.id}`);
    ...
    const consistencyScore = totalChecks === 0 ? 1.0 : parseFloat((1 - ((duplicatesRemoved + contradictionsResolved) / totalChecks)).toFixed(4));
  ...
  public addMemory(fact: string, source: string, tags: string[] = []): MemoryBlock {
    const newMemory: MemoryBlock = {
      id: "M16-" + Math.floor(Math.random() * 1000).toString().padStart(3, "0"),
      fact,
      source,
      confidence: 0.95,
      timestamp: Date.now(),
