...
export class CrystalMemoryV2 {
  ...
  public integrateConcept(
    ...
    const conflicting = this.memoryGraph.filter(
      (n) =>
    ...
    if (conflicting.length > 0) {
      ...
      deduplicationLog.push(
        `Contradiction flagged for "${term}": New description conflicts with existing records.`,
      );
    }

    // 2. Duplicate checking / merging similar concepts
    const matchIdx = this.memoryGraph.findIndex(
      (n) =>
        n.term.toLowerCase() === term.toLowerCase() ||
