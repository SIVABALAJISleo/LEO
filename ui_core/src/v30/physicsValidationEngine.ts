...
export class PhysicsValidationEngine {
  verifyConstraints(actionLabel: string, params: PhysicalParameters): PhysicsAudit {
    ...
    if (params.availableEnergyJoules !== undefined && kineticEnergy > params.availableEnergyJoules) {
      violations.push(`Kinetic energy (${kineticEnergy.toFixed(1)}J) exceeds available source capacity (${params.availableEnergyJoules}J).`);
    }

    // Rule 2: G-force safety check
    if (params.maxAccelerationG > 4.5) {
      violations.push(`Acceleration parameter (${params.maxAccelerationG}G) violates structural integrity bounds of 4.5G.`);
    }

    // Rule 3: Friction sliding threshold check
    if (params.velocityMS > 30.0 && params.frictionCoeff < 0.2) {
      violations.push(`High speed (${params.velocityMS} m/s) with low friction coeff (${params.frictionCoeff}) risks uncontrollable drift.`);
    }
    ...
    const plausibilityScore = violations.length === 0 ? 1.0 : Math.max(0.1, 1.0 - violations.length * 0.3);
