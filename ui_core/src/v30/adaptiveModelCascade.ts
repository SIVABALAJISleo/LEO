...
export class AdaptiveModelCascade {
  evaluateQuery(query: string): CascadeStep[] {
    ...
    const isVeryComplex = query.length > 50 || query.toLowerCase().includes("proof") || query.toLowerCase().includes("scientific");
    const isPhysicsHeavy = query.toLowerCase().includes("constraint") || query.toLowerCase().includes("motion");
