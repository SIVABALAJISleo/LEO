  ...
  predictRequiredExperts(query: string): PredictionTelemetry {
    ...
    const weights: ExpertWeight[] = this.experts.map(exp => {
      ...
      if (exp.expertId === "exp-0") {
        if (lower.includes("why") || lower.includes("explain") || lower.includes("logic")) score += 0.7;
      }
      if (exp.expertId === "exp-1") {
        if (lower.includes("math") || lower.includes("compute") || lower.includes("equation") || lower.includes("sum")) score += 0.8;
      }
      if (exp.expertId === "exp-2") {
        if (lower.includes("code") || lower.includes("function") || lower.includes("typescript") || lower.includes("bug")) score += 0.85;
      }
      if (exp.expertId === "exp-3") {
        if (lower.includes("write") || lower.includes("translate") || lower.includes("summarize")) score += 0.6;
      }
      if (exp.expertId === "exp-4") {
        if (lower.includes("where") || lower.includes("find") || lower.includes("database") || lower.includes("knowledge")) score += 0.75;
      }
      return {
