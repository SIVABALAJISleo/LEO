...
const toVerification = (citations: EvidenceCitation[], conf: number): VerificationStatus => {
  const verified = citations.filter((c) => c.relevanceScore >= 0.8).length;
  if (verified >= 3 && conf >= 0.9) return "VERIFIED";
  if (verified >= 1 && conf >= 0.7) return "PARTIALLY_VERIFIED";
  if (conf < 0.5) return "CONTESTED";
  return "UNVERIFIED";
};
...
export class EnterpriseTrustEngine {
  ...
  wrap(query: string, rawAnswer: string, baseConfidence: number): EnterpriseAnswer {
    ...
    const sourceTypes: EvidenceCitation["sourceType"][] = [
      "GraphRAG",
      "Memory",
      "Search",
      "Database",
    ...
    const conf = Math.min(
      0.99,
      baseConfidence + citations.filter((c) => c.relevanceScore > 0.85).length * 0.01,
    );
