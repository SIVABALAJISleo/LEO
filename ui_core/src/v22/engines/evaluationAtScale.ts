...
const DOMAIN_CONFIG: Record<EvalDomain, { tasks: number; baseAccuracy: number; baseHalluc: number }> = {
...
export class EvaluationAtScale {
  ...
  runEvaluation(version = 'v22.0'): ScaleEvaluationReport {
    ...
    const domainBenchmarks: DomainBenchmark[] = (Object.keys(DOMAIN_CONFIG) as EvalDomain[]).map(domain => {
      const cfg = DOMAIN_CONFIG[domain];
      const noise = (Math.random() - 0.5) * 0.015;
      const accuracy = Math.min(0.999, Math.max(0.80, cfg.baseAccuracy + noise));
