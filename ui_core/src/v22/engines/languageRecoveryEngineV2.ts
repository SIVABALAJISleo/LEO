...
export class LanguageRecoveryEngineV2 {
  ...
  recover(rawInput: string): LanguageRecoveryV2Result {
    ...
    words.forEach((word, idx) => {
      ...
      if (TANGLISH_MAP[lw] !== undefined) {
        ...
        repairs.push({ original: word, repaired: replacement, issueType: 'tanglish', confidence: 0.93 });
    ...
    repairedWords.forEach((word, idx) => {
      ...
      if (SLANG_MAP[lw]) {
        hasSlang = true;
        repairs.push({ original: word, repaired: SLANG_MAP[lw], issueType: 'slang', confidence: 0.96 });
    ...
    repairedWords.forEach((word, idx) => {
      ...
      if (TYPO_MAP[lw]) {
        hasTypos = true;
        repairs.push({ original: word, repaired: TYPO_MAP[lw], issueType: 'typo', confidence: 0.98 });
    ...
    const isBrokenEnglish = words.length < 4 && !/\b(is|are|do|does|how|what|why|can|will)\b/i.test(lower);
    if (isBrokenEnglish) {
    ...
    const isAmbiguous = ambiguousTerms.some(t => lower.includes(` ${t} `) || lower.startsWith(`${t} `));
  ...
  getStats() {
    return {
      ...
      averageIntentAccuracy: this.totalProcessed > 0 ? this.totalIntentScore / this.totalProcessed : 0,
    };
  }
}
