  ...
  public generateResearchProposal(topic: string): ResearchReport {
    ...
    if (topicLower.includes("ai") || topicLower.includes("model") || topicLower.includes("mamba")) {
      literatureReviewed.push(
        {
          ...
          year: 2023,
          coreContribution:
            "Introduces linear-time selective state space models matching transformers in accuracy.",
          relevanceScore: 0.98,
        },
        {
          ...
          year: 2025,
          coreContribution:
            "Exposes memory-efficient 4-bit integer weights for mobile iGPU shaders.",
          relevanceScore: 0.92,
        },
      );
      ...
      literatureReviewed.push({
        ...
        year: 2024,
        coreContribution:
          "Details off-chain verification methods for high-frequency billing platforms.",
        relevanceScore: 0.9,
      });
