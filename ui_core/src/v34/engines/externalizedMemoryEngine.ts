...
export class ExternalizedMemoryEngine {
  ...
  public queryExternalKnowledge(query: string): RetrievalSummary {
    ...
    const retrievedFacts = this.localKnowledgeBase.filter(
      (f) =>
    ...
    const knowledgeRetrievalRatePct =
      retrievedFacts.length > 0 ? parseFloat((95.0 + Math.random() * 4.9).toFixed(2)) : 95.0;
