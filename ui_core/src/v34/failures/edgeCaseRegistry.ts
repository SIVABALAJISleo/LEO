...
export class EdgeCaseRegistry {
  private registry = new Map<string, EdgeCaseRecord>([
    ["oom-65k", { caseId: "ec-oom-65k", triggerCondition: "Context length > 65536 on FP16 CPU execution", reproductionHash: "h-ec-01-v34", impactLevel: "critical" }],
    ["precision-degrad", { caseId: "ec-precision", triggerCondition: "Clamping weights to 1-bit on logical reasoning tasks", reproductionHash: "h-ec-02-v34", impactLevel: "warning" }]
  ]);

  registerCase(trigger: string, hash: string, impact: "critical" | "warning" | "low"): EdgeCaseRecord {
