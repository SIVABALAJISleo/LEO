...
interface AuditEntry {
  ...
  durationMs: number;
  result:
    "success" | "degraded" | "failed" | "rejected" | "circuit-open" | "cached" | "approximated";
  attempts: number;
  error?: string;
  quality?: string;
}
...
export class ReliabilityOrchestrator {
  ...
  private circuitBreakers = new Map<
    string,
    { state: CircuitState; failures: number; lastFailure: number }
  >();
  ...
  async executeWithTrace<T, R>(
    ...
    if (this.isRateLimited(actionType, config)) {
      const finalResult = await this.handleFailureChain<T, R>(
        actionType,
        payload,
        startTime,
  ...
  private async handleFailureChain<T, R>(
    actionType: string,
    payload: T,
    startTime: number,
    lastError: Error,
  ...
  private getHandler(actionType: string): ActionHandler {
    ...
    if (!handler) {
      console.warn(
        `[Reliability] Unknown action type: ${actionType}. Falling back to ai_inference.`,
      );
