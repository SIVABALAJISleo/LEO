...
export function getStatusColor(status: GpuJobStatus): string {
  switch (status) {
    ...
    case 'too_large': return 'text-red-400';
    case 'cancelled': return 'text-muted-foreground';
    default: return 'text-muted-foreground';
  }
}

export function getStatusBadgeVariant(status: GpuJobStatus): 'default' | 'secondary' | 'destructive' | 'outline' {
  switch (status) {
    case 'completed': return 'default';
    case 'running': return 'default';
    case 'failed':
    case 'too_large': return 'destructive';
    case 'pending':
    case 'queued':
...
export function getStatusMessage(status: GpuJobStatus, position?: number): string {
  switch (status) {
    case 'pending': return 'Your job is being prepared...';
    case 'queued': return position ? `Your job is queued (Position #${position})` : 'Your job is queued';
    case 'running': return 'Your job is running on HYPER Engine...';
    case 'paused': return 'Job paused for thermal protection';
    case 'completed': return 'Completed! Download your results below.';
    case 'failed': return 'Job failed. Check error details.';
    case 'too_large': return 'Rejected: Job requires too much GPU memory';
