...
class HumanAuthorityIntegration {
  ...
  createRequirement(params: {
    ...
    console.log(
      `[AuthorityIntegration] Created requirement ${requirementId} for ${params.authorityType}`,
    );
    return requirement;
  }
  ...
  getSystemState(taskId: string): AuthoritySystemState {
    const taskRequirements = Array.from(this.requirements.values()).filter(
      (r) => r.taskId === taskId,
    );
  ...
  receiveAuthorityInput(
    ...
    const responseTimeMs =
      new Date(requirement.receivedAt).getTime() -
    ...
    console.log(
      `[AuthorityIntegration] Authority input received and verified for ${requirementId}`,
    );
  ...
  getPendingRequirements(): AuthorityRequirement[] {
    return Array.from(this.requirements.values()).filter((r) => r.status === "pending");
  }
