...
class PricingEngineSystem {
  ...
  private readonly MIN_GROSS_MARGIN = 0.4; // 40% minimum
  ...
  readonly tiers: Record<string, PricingTier> = {
    pro: {
      ...
      restrictions: ["No SLA guarantee", "Non-deterministic routing", "Shared resource pool"],
    },
    heavy: {
      id: "heavy",
      name: "HYPER Heavy",
  ...
  isClaimAllowed(claim: string): boolean {
    ...
    return !this.FORBIDDEN_CLAIMS.some((forbidden) => lowerClaim.includes(forbidden));
  }
   ...
   */
  calculateMarginStatus(revenue: number, costs: number): MarginStatus {
