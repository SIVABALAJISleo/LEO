...
class IntelligenceCompositionEngine {
  ...
  private retrievalSystems: Map<RetrievalSource, (query: string) => Promise<RetrievalResult>> = new Map();
  ...
  async compose(query: string, context?: unknown): Promise<ComposedAnswer> {
    ...
    const avgExpertConfidence = validOutputs.length > 0
  ...
  private combineOutputs(
    ...
    for (const output of outputs) {
      ...
      parts.push(`[${output.domain}] (${(output.confidence * 100).toFixed(0)}% confidence): ${resultStr}`);
    }
