...
class AlgorithmicShortcutRegistryEngine {
  ...
  findShortcut(
    ...
    for (const shortcut of this.shortcuts.values()) {
      const typeMatches = shortcut.applicableWorkloads.some(w => 
        type.includes(w) || w.includes(type)
      );
  ...
  getStats() {
    return {
      ...
      applicationRate: this.stats.totalLookups > 0 
        ? this.stats.shortcutsApplied / this.stats.totalLookups 
        : 0,
    };
  }
