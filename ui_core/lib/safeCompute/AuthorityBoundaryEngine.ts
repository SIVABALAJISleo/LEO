...
class AuthorityBoundaryEngine {
  ...
  classify(request: {
    ...
    const searchText =
      `${request.type} ${request.description} ${request.domain || ""}`.toLowerCase();
    ...
    switch (boundaryType) {
      ...
      default:
        return {
          ...
          allowedActions: ["Full software execution", "Autonomous processing", "Result delivery"],
          forbiddenActions: [],
  ...
  isActionAllowed(boundaryType: AuthorityBoundaryType, action: string): boolean {
    ...
    return !tempClassification.forbiddenActions.some((f) =>
