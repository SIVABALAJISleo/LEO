...
export type DiscoveryDomain =
  "scientific" | "engineering" | "business" | "medical" | "safety" | "optimization";
...
class GuidedDiscoveryEngine {
  ...
  private calculateInitialConfidence(variables: Variable[], assumptions: string[]): number {
    // Base confidence
    let confidence = 0.5;
  ...
  private predictOutcome(hypothesis: Hypothesis, conditions: Record<string, unknown>): unknown {
    ...
    if (independent && dependent) {
      ...
      if (typeof independentValue === "number" && independent.expectedRange) {
        const normalized =
          (independentValue - independent.expectedRange.min) /
  ...
  private identifyResources(hypothesis: Hypothesis, steps: ExperimentStep[]): string[] {
