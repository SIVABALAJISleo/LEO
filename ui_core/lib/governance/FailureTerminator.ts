...
import { TerminalAction, PipelineContext, PipelineStage } from "./types";
...
export class FailureTerminator {
   ...
   */
  terminate(error: unknown, context: PipelineContext): TerminationResult {
    ...
    console.error(`[FailureTerminator] ${result.action} at ${result.stage}: ${result.reason}`);
