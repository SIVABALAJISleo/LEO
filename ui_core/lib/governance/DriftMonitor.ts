...
import { DistributionSnapshot, DriftReport } from "./types";
...
export class DriftMonitor {
  ...
  private updateHistorical(domain: string, embeddings: number[][]): void {
    ...
    const merged: DistributionSnapshot = {
      mean: existing.mean.map((m, i) => m * (1 - alpha) + (dist.mean[i] || 0) * alpha),
      variance: existing.variance.map((v, i) => v * (1 - alpha) + (dist.variance[i] || 0) * alpha),
