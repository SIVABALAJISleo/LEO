...
export const PerformanceOverview = ({ metrics, activeJobs }: PerformanceOverviewProps) => {
  const stats = useMemo(() => {
    ...
    const recentMetrics = metrics.filter(
      m => new Date(m.recorded_at).getTime() > oneHourAgo
    );

    const latencyMetrics = recentMetrics.filter(m => m.metric_name === 'latency' || m.latency_ms);
    const avgLatency = latencyMetrics.length > 0
      ? latencyMetrics.reduce((sum, m) => sum + (m.latency_ms || m.metric_value || 0), 0) / latencyMetrics.length
      : 0;

    const throughputMetrics = recentMetrics.filter(m => m.metric_name === 'throughput' || m.throughput_rps);
    const avgThroughput = throughputMetrics.length > 0
      ? throughputMetrics.reduce((sum, m) => sum + (m.throughput_rps || m.metric_value || 0), 0) / throughputMetrics.length
      : 0;

    const cacheMetrics = recentMetrics.filter(m => m.metric_name === 'cache_hit_ratio' || m.cache_hit_ratio);
    const avgCacheHit = cacheMetrics.length > 0
      ? cacheMetrics.reduce((sum, m) => sum + (m.cache_hit_ratio || m.metric_value || 0), 0) / cacheMetrics.length
      : 0;

    // Calculate average speedup from jobs
    const jobsWithSpeedup = activeJobs.filter(j => j.speedup);
    const avgSpeedup = jobsWithSpeedup.length > 0
