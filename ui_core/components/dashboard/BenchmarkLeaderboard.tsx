...
export const BenchmarkLeaderboard: React.FC = () => {
  ...
  const models = Object.keys(results.models || {}).map(key => ({
    name: key,
    ...results.models[key]
        ...
        <div className="flex justify-between items-center mb-6">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-1">
              Real-time API benchmarking against MSR dataset. Status: <span className={`font-bold uppercase ${results.status === 'running' ? 'text-amber-400' : 'text-emerald-400'}`}>{results.status}</span>
            </p>
          </div>
          <button 
            ...
            <tbody className="divide-y divide-slate-800 text-slate-200">
              {models.map((model, index) => (
                <tr key={model.name} className={model.name.includes("LEO") ? "bg-blue-500/5" : ""}>
                  <td className={`p-4 font-bold ${model.name.includes("LEO") ? 'text-blue-400' : 'text-slate-400'}`}>
                    {index + 1}
                  </td>
                  ...
                  <td className={`p-4 font-bold ${model.accuracy >= 90 ? 'text-emerald-400' : model.accuracy >= 50 ? 'text-amber-400' : 'text-slate-500'}`}>
        ...
        {results.history && results.history.length > 0 && (
           <div className="mt-8 border-t border-slate-800 pt-6">
             ...
             <div className="space-y-3">
               {results.history.slice(-3).reverse().map((h: any, i: number) => (
                 <div key={i} className="bg-[#020813] border border-slate-800 p-4 rounded text-xs text-slate-400">
                   <p className="text-slate-300 font-semibold mb-2">Q: {h.query}</p>
                   <div className="grid grid-cols-2 gap-2 text-[10px]">
                     <div>LEO: {h.leo.accuracy.toFixed(0)}% ({h.leo.latency.toFixed(0)}ms)</div>
                     <div>GPT: {h.gpt.accuracy.toFixed(0)}% ({h.gpt.latency.toFixed(0)}ms)</div>
                     <div>Claude: {h.claude.accuracy.toFixed(0)}% ({h.claude.latency.toFixed(0)}ms)</div>
                     <div>Gemini: {h.gemini.accuracy.toFixed(0)}% ({h.gemini.latency.toFixed(0)}ms)</div>
                   </div>
                 </div>
               ))}
             </div>
