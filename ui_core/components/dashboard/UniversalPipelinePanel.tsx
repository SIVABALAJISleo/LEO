...
import {
  ...
  PipelineStats,
  PipelineStep,
} from "@/lib/safeCompute/UniversalDecisionPipeline";
      ...
      <CardContent className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          <div className="grid grid-cols-2 gap-2 text-xs">
            {[
              {
                key: "completed_via_outcome_replacement",
                label: "Outcome Replaced",
                icon: Replace,
                color: "text-primary",
              },
              {
                key: "completed_via_reuse",
                label: "Reused",
                icon: Database,
                color: "text-blue-500",
              },
              {
                key: "completed_via_approximation",
                label: "Approximated",
                icon: Layers,
                color: "text-purple-500",
              },
              {
                key: "completed_via_perceived_realtime",
                label: "Perceived RT",
                icon: Clock,
                color: "text-cyan-500",
              },
              {
                key: "completed_via_local_execution",
                label: "Local (Verified)",
                icon: Cpu,
                color: "text-green-500",
              },
              {
                key: "completed_via_delegation",
                label: "Delegated",
                icon: Cloud,
                color: "text-orange-500",
              },
              {
                key: "physics_limited_explained",
                label: "Physics-Limited",
                icon: XCircle,
                color: "text-red-500",
              },
        ...
        <div className="bg-muted/30 rounded-lg p-3 text-xs font-mono">
          ...
          <div className="space-y-1 text-muted-foreground">
            <div>
              GPU replacement: <span className="text-destructive font-medium">❌ NOT CLAIMED</span>
            </div>
            <div>
              GPU dependency neutralized: <span className="text-primary font-medium">✅ YES</span>
            </div>
            <div>
              Practical coverage:{" "}
              <span className="text-primary font-medium">{auditReport.coveragePercent}%</span>
            </div>
            <div>
              Remaining: <span className="text-muted-foreground">optional, rare, delegatable</span>
            </div>
          </div>
        </div>
        ...
        {stats && stats.totalProcessed > 0 && (
          <div className="text-center text-xs text-muted-foreground">
            {stats.totalProcessed} workloads processed • Last updated{" "}
