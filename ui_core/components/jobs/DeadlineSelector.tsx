...
import { deadlineScheduler, type DeadlineEstimate, type QualityLevel } from '@/lib/safeCompute/DeadlineScheduler';
    ...
    <Card className={cn('border-border', className)}>
      ...
      <CardContent className="space-y-4">
        ...
        <div className={cn(
          ...
          {estimate.canMeetDeadline ? (
            ...
            <>
              ...
              <span>Full quality needs more time ({timeRange.min} – {timeRange.max})</span>
            </>
          )}
        </div>
        ...
        <RadioGroup
          ...
          className="space-y-2"
        >
          {estimate.alternatives.filter(a => a.available).map((alt) => (
            <div
              key={alt.id}
              className={cn(
            ...
            >
              ...
              <div className="text-right text-xs text-muted-foreground">
                ...
                <p>{Math.round(alt.confidenceRange[0] * 100)}–{Math.round(alt.confidenceRange[1] * 100)}%</p>
              </div>
            </div>
          ))}
        </RadioGroup>
        ...
        <Button onClick={handleSubmit} className="w-full">
          Continue with {selectedQuality === 'approximate' ? 'Quick' : selectedQuality === 'reduced' ? 'Balanced' : 'Full'} Quality
        </Button>
      </CardContent>
    </Card>
  );
