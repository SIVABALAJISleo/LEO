  ...
  return (
    ...
    >
      ...
      <div
        style={{
          ...
          background: isActive
            ...
            : "radial-gradient(circle, rgba(255, 0, 85, 0.2) 0%, transparent 70%)",
          filter: "blur(20px)",
          zIndex: 0,
        }}
      ></div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px", zIndex: 1 }}>
        <StatBox
          label="Local Tensors Uploaded"
          value={telemetry.localContributions.toLocaleString()}
          color="#00d2ff"
        />
        <StatBox
          label="Global Model Version"
          value={`v42.${telemetry.globalModelVersion}`}
          color="#bb00ff"
        />
        <StatBox
          label="Vaccines Synthesized"
          value={telemetry.vaccinesGenerated.toLocaleString()}
          color="#ffaa00"
        />
        <StatBox
          label="Global Improvement"
          value={`+${telemetry.globalImprovementPercent.toFixed(2)}%`}
          color="#00ff88"
        />
      </div>
      ...
      <div style={{ zIndex: 1, marginTop: "8px" }}>
        ...
        >
          ...
          <span>
            {((telemetry.syntheticGeneratedToday / telemetry.dailySyntheticQuota) * 100).toFixed(
              1,
            ) || 0}
            %
          </span>
...
const StatBox: React.FC<{ label: string; value: string | number; color: string }> = ({
  ...
  >
    ...
    <span style={{ fontSize: "1.5rem", fontWeight: 700, color, textShadow: `0 0 10px ${color}40` }}>
      {value}
    </span>
  </div>
);
