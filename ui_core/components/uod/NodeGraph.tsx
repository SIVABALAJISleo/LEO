...
export const NodeGraph = () => {
  ...
  const compileAndRun = async () => {
    ...
    try {
      ...
      await globalDuckDB.runQuery(
        'CREATE TABLE IF NOT EXISTS sales_data AS SELECT * FROM (SELECT 100.5 AS revenue, "A" AS user_id UNION ALL SELECT 2000.0, "B")',
      );
  ...
  return (
    <div className="w-full h-full relative bg-slate-950">
      ...
      >
        ...
        <Panel
          position="top-right"
          className="bg-slate-900/90 p-4 rounded-xl border border-slate-700 w-80 text-white shadow-2xl backdrop-blur-md"
        >
        ...
        <Panel
          ...
          className="bg-slate-900/90 p-4 rounded-xl border border-slate-700 max-w-lg min-h-[100px] text-white shadow-2xl backdrop-blur-md overflow-auto max-h-[300px]"
        >
          <h3 className="text-green-400 font-bold mb-2 uppercase text-xs tracking-widest">
            Result Preview
          </h3>
          {results.length > 0 ? (
              ...
              <thead>
                <tr className="border-b border-slate-700">
                  {Object.keys(results[0]).map((k) => (
                    <th key={k} className="text-left p-1">
                      {k}
                    </th>
                  ))}
                </tr>
