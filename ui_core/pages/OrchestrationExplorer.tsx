      ...
      if (activeTab === "moe") {
        ...
        data = await (async (q: string) => {
          ...
          return {
            ...
            novelty: {
              state: noveltyResult.state,
              similarity: noveltyResult.similarity.toFixed(3),
            },
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      <div className="flex flex-col space-y-2">
        ...
        <p className="text-muted-foreground underline decoration-primary/30">
          Unified Interface for Hybrid CPU Engines —{" "}
          <span className="text-primary font-bold">Remote Core Active</span>
        </p>
      </div>
          ...
          <Card className="border-primary/20 bg-card/50 backdrop-blur-sm">
            ...
            <CardContent>
              <div className="flex gap-2">
                ...
                <Button onClick={handleRun} disabled={loading} className="h-12 px-6 shadow-glow">
                  {loading ? (
                    "Processing..."
                  ) : (
                    <>
              ...
              <Tabs value={activeTab} onValueChange={setActiveTab} className="mt-6">
                <TabsList className="grid w-full grid-cols-3 bg-muted/50">
                  <TabsTrigger
                    value="unified"
                    className="data-[state=active]:bg-primary pulse-on-hover"
                  >
              ...
              <CardTitle className="text-lg flex justify-between items-center">
                ...
                <div className="flex gap-2 items-center">
                  {coreData?.sdgp_active && (
                    <Badge
                      variant="outline"
                      className="text-cyan-400 border-cyan-400/30 animate-pulse"
                    >
                      SDGP BYPASS ACTIVE
                    </Badge>
                  )}
                ...
                {results ? (
                  <div className="space-y-4">
                    <div className="bg-primary/5 p-4 rounded-lg border border-primary/10">
                      <p className="text-xs font-mono text-muted-foreground mb-2 uppercase tracking-tighter">
                        Result Context
                      </p>
        ...
        <div className="space-y-6">
          ...
          <Card className="border-cyan-500/30 bg-cyan-500/5 backdrop-blur-xl">
            ...
            <CardContent className="space-y-4">
              <div className="space-y-1">
                ...
                <Progress
                  value={coreData ? 100 : 0}
                  className="h-1 bg-cyan-950"
                  indicatorClassName="bg-cyan-400 shadow-[0_0_10px_rgba(34,211,238,0.5)]"
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-2">
                <div className="p-2 bg-black/40 rounded border border-white/5 text-center">
                  ...
                  <p className="text-sm font-bold text-cyan-400">
                    {coreData?.ray_logic_depth || "0"}
                  </p>
                </div>
                <div className="p-2 bg-black/40 rounded border border-white/5 text-center">
                  <p className="text-[9px] text-muted-foreground uppercase">Latency</p>
                  <p className="text-sm font-bold text-cyan-400">
                    {coreData?.sdgp_latency_ms?.toFixed(2) || "0.00"}ms
                  </p>
                </div>
              </div>
              ...
              <div className="space-y-2 text-[10px] font-mono">
                <div className="flex justify-between items-center p-1.5 bg-black/20 rounded">
                  ...
                  <Badge
                    variant="secondary"
                    className={`text-[8px] h-4 ${coreData?.dlss_s_active ? "bg-cyan-500/20 text-cyan-400" : "opacity-30"}`}
                  >
              ...
              {[
                {
                  ...
                  icon: Code,
                  color: "text-blue-500",
                  keywords: "code, optimize, refactor",
                },
                {
                  name: "Vision Boundary",
                  icon: Eye,
                  color: "text-green-500",
                  keywords: "see, look, detect, render",
                },
                {
                  name: "Logic Solver",
                  icon: Brain,
                  color: "text-purple-500",
                  keywords: "solve, calculate, prove",
                },
                {
                  name: "Vector Store",
                  icon: Search,
                  color: "text-yellow-500",
                  keywords: "find, search, retrieve",
                },
              ].map((track) => (
                <div
                  key={track.name}
          ...
          <Card className="bg-primary/5 border-primary/30 border-dashed">
            ...
            <CardContent className="text-xs text-muted-foreground leading-relaxed">
              <strong>GOVERNED</strong> tab now routes to the 5090-equivalent Python backend. Try
              queries like <em>"Render a complex scene with 64-depth ray logic"</em> or{" "}
              <em>"Calculate average of 10,20,30,40,50"</em>.
            </CardContent>
          </Card>
        </div>
      </div>
