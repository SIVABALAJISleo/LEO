        ...
        const fetchData = async () => {
            try {
                const [s, h] = await Promise.all([
                    hyperClient.getStatus(),
                    hyperClient.getHealth()
                ]);
                setStatus(s);
                setHealth(h);
            } catch (err) {
        ...
        <div className="p-6 space-y-6 bg-background min-h-screen text-foreground">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <Card className="bg-card/50 border-primary/20 shadow-glow-sm">
                    ...
                    <CardContent>
                        <div className="text-2xl font-bold">{(status?.metrics.latency_avg || 0).toFixed(4)}s</div>
                    </CardContent>
                </Card>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-8">
                <Card className="bg-card/50 border-primary/20">
                    ...
                    <CardContent className="space-y-4">
                        {['RAG Optimized', 'MoE Router', 'Vision (Spatial)', 'Physics (Baked)'].map((engine) => (
                            <div key={engine} className="flex justify-between items-center p-3 rounded-lg bg-muted/30">
                                <span>{engine}</span>
                        ...
                        <div className="space-y-6">
                            ...
                            <div>
                                <div className="flex justify-between mb-2">
                                    ...
                                    <span className="text-sm font-bold">{status?.hardware.memory_percent.toFixed(1)}%</span>
                                </div>
                                <div className="h-2 w-full bg-muted rounded-full overflow-hidden">
                                    <div
                            ...
                            <div>
                                <div className="flex justify-between mb-2">
                                    ...
                                    <span className="text-sm font-bold">{status?.hardware.disk_percent.toFixed(1)}%</span>
                                </div>
                                <div className="h-2 w-full bg-muted rounded-full overflow-hidden">
                                    <div
