    ...
    <div className="space-y-6 p-6">
      <div>
        ...
        <p className="text-muted-foreground">
          Real-time cost tracking, forecasts, and budget controls
        </p>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            <p
              className={`text-2xl font-bold ${totalBudget > 0 && totalSpent / totalBudget > 0.8 ? "text-destructive" : ""}`}
            >
      ...
      <Tabs defaultValue="budgets">
                ...
                <CardContent className="flex justify-between items-center py-3">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      ${b.spent_amount?.toFixed(2) || 0} / ${b.total_budget}
                    </p>
                  </div>
                  <div className="flex items-center gap-2">
                    <div className="w-32 bg-muted rounded-full h-2">
                      <div
                        ...
                        style={{
                          width: `${Math.min(((b.spent_amount || 0) / b.total_budget) * 100, 100)}%`,
                        }}
                      />
                    </div>
        ...
        <TabsContent value="transactions">
          {transactions.length === 0 ? (
            <EmptyState title="No transactions" description="No cost transactions recorded" />
          ) : (
            transactions.slice(0, 20).map((t) => (
              <Card key={t.id} className="mb-2">
                <CardContent className="flex justify-between py-3">
                  <div>
                    <p className="font-medium">{t.resource_type}</p>
                    <p className="text-sm text-muted-foreground">
        ...
        <TabsContent value="predictions">
          {predictions.length === 0 ? (
            <EmptyState
              ...
              description="Cost predictions will appear here"
              icon={TrendingUp}
            />
          ) : (
            predictions.map((p) => (
              <Card key={p.id} className="mb-2">
                <CardContent className="grid grid-cols-4 gap-4 py-3">
                  <div>
                    <span className="text-muted-foreground">Resource:</span> {p.resource_type}
                  </div>
                  <div>
                  ...
                  <div>
                    <span className="text-muted-foreground">Period:</span> {p.prediction_period}
                  </div>
                  <div>
                    <span className="text-muted-foreground">Date:</span> {p.prediction_date}
                  </div>
                </CardContent>
              </Card>
            ))
