  ...
  const stats = useMemo(
    () => ({
      ...
      avgFailoverTime:
        failovers.length > 0
          ? Math.round(
              failovers.reduce((sum, f) => sum + (f.duration_ms || 0), 0) / failovers.length,
            )
          : 0,
    }),
  ...
  return (
    <div className="space-y-6 p-6">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={(v) => setNewBackup({ ...newBackup, backup_type: v })}
                  >
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
                ...
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={(v) => setNewBackup({ ...newBackup, region: v })}
                  >
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
          ...
          <Dialog open={isFailoverOpen} onOpenChange={setIsFailoverOpen}>
            <DialogTrigger asChild>
              <Button variant="destructive">
                <Shield className="mr-2 h-4 w-4" /> Trigger Failover
              </Button>
            </DialogTrigger>
            <DialogContent>
              <DialogHeader>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <Select
                    value={failoverData.from_region}
                    onValueChange={(v) => setFailoverData({ ...failoverData, from_region: v })}
                  >
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
                ...
                <div>
                  ...
                  <Select
                    value={failoverData.to_region}
                    onValueChange={(v) => setFailoverData({ ...failoverData, to_region: v })}
                  >
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
                ...
                <div>
                  ...
                  <Input
                    value={failoverData.trigger_reason}
                    onChange={(e) =>
                      setFailoverData({ ...failoverData, trigger_reason: e.target.value })
                    }
                    placeholder="Failover reason"
                  />
                </div>
                <Button onClick={handleFailover} variant="destructive" className="w-full">
                  Initiate Failover
                </Button>
              </div>
            </DialogContent>
      ...
      <div className="grid gap-4 md:grid-cols-4">
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm">Avg Failover Time</CardTitle>
          </CardHeader>
          <CardContent>
      ...
      <Tabs defaultValue="backups">
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                          {b.region} • {formatBytes(b.size_bytes || 0)}
                        </p>
                      </div>
                    </div>
                    <div className="flex items-center gap-4">
        ...
        <TabsContent value="failovers" className="space-y-4">
          {failovers.length === 0 ? (
            <EmptyState
              title="No failover events"
              description="Failover history will appear here"
              icon={Globe}
            />
          ) : (
            <div className="space-y-2">
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      {f.data_loss_bytes && f.data_loss_bytes > 0 && (
                        <Badge variant="destructive">
                          Data Loss: {formatBytes(f.data_loss_bytes)}
                        </Badge>
                      )}
                    </div>
                  </CardContent>
                ...
                <Card key={i.id} className={i.status === "active" ? "border-destructive" : ""}>
                  <CardContent className="py-4">
                    <div className="flex justify-between items-start">
                      <div className="flex items-center gap-4">
                        <AlertTriangle
                          className={`h-8 w-8 ${i.severity === "critical" ? "text-destructive" : i.severity === "high" ? "text-orange-500" : "text-yellow-500"}`}
                        />
                        <div>
                    ...
                    <div className="mt-4 flex justify-between items-center">
                      <div className="flex items-center gap-2 text-sm text-muted-foreground">
                        ...
                        {i.resolved_at && (
                          <span>• Resolved: {new Date(i.resolved_at).toLocaleString()}</span>
                        )}
                      </div>
                      {i.status !== "resolved" && (
                        <Button
                          size="sm"
                          onClick={() => resolveIncident(i.id, "Manual resolution")}
                        >
                          Resolve
                        </Button>
                      )}
