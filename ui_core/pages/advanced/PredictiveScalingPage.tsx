...
import {
  usePredictiveScalingData,
  PREDICTION_TYPES,
  TIME_HORIZONS,
} from "@/hooks/usePredictiveScalingData";
...
const PredictiveScalingPage = () => {
  ...
  const chartData = predictions
    .filter((p) => p.prediction_type === selectedType)
    .map((p) => ({
    ...
    <div className="space-y-6 p-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          <Button onClick={() => generatePredictions(selectedType, selectedHorizon)}>
        ...
        <Card>
          ...
          <CardContent>
            <p className="text-2xl font-bold">
              {avgAccuracy ? `${avgAccuracy.toFixed(1)}%` : "N/A"}
            </p>
          </CardContent>
        </Card>
        <Card>
          ...
          <CardContent>
            <p className="text-2xl font-bold text-green-500">${getTotalSavings().toFixed(2)}</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <Card>
          <CardHeader>
            <CardTitle>Proposed Scaling Actions</CardTitle>
          </CardHeader>
                ...
                >
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      {action.previous_count} → {action.new_count} • {action.trigger_reason}
                    </p>
                  </div>
                  <div className="flex gap-2">
                    <Button size="sm" onClick={() => executeScalingAction(action.id)}>
                      <Play className="h-4 w-4" />
                    </Button>
                    <Button
                      size="sm"
                      variant="outline"
                      onClick={() => cancelScalingAction(action.id)}
                    >
                      <X className="h-4 w-4" />
                    </Button>
                  </div>
