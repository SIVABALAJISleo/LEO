...
import {
  ...
  SPECIALIZATIONS,
  DISTILLATION_STAGES,
} from "@/hooks/useDistillationData";
...
const DistillationPage = () => {
  ...
  const handleCreate = async () => {
    ...
    await createDistilledModel({
      name: newModelName,
      teacher_model_id: selectedTeacher || undefined,
    ...
    <div className="space-y-6 p-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Create optimized student models from teacher models
          </p>
        </div>
      </div>
      ...
      <Tabs defaultValue="models">
        ...
        <TabsContent value="models" className="space-y-4">
          {models.length === 0 ? (
            <EmptyState
              title="No distilled models"
              description="Create your first distilled model to get started"
              icon={Brain}
            />
          ) : (
                ...
                return (
                  <Card key={model.id}>
                    <CardHeader>
                      ...
                      <CardDescription>
                        {model.specialization || "General"} • Stage {model.current_stage}/5
                      </CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-4">
                      <div className="grid grid-cols-3 gap-2 text-center text-sm">
                        <div>
                          ...
                          <p className="font-bold">{model.accuracy ? `${model.accuracy}%` : "-"}</p>
                        </div>
                        <div>
                        ...
                        <div>
                          ...
                          <p className="font-bold">
                            {model.compression_ratio ? `${model.compression_ratio}x` : "-"}
                          </p>
                        </div>
                      </div>
                      <div className="space-y-1">
                        ...
                        <div className="flex gap-1">
                          {DISTILLATION_STAGES.map((s) => (
                            <div
                              key={s.stage}
                              className={`flex-1 h-2 rounded ${model.current_stage >= s.stage ? "bg-primary" : "bg-muted"}`}
                              title={s.name}
                            />
                          ))}
                        </div>
                      ...
                      {sweetSpot && (
                        <div className="p-2 bg-primary/10 rounded text-sm">
                          ...
                          Sweet Spot: Stage {sweetSpot.stage} - {sweetSpot.reason}
                        </div>
                      )}
                      <div className="flex gap-2">
                        ...
                        <Button
                          ...
                          variant="outline"
                          onClick={() =>
                            rollbackToStage(model.id, Math.max(1, model.current_stage - 1))
                          }
                        >
              ...
              <Card key={job.id}>
                ...
                <CardContent>
                  ...
                  <p className="text-sm text-muted-foreground mt-2">
                    Stage {job.stage} • Priority {job.priority}
                  </p>
                </CardContent>
              </Card>
            ))
        ...
        <TabsContent value="create">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Define a new student model for knowledge distillation
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <Input
              ...
              <Select value={selectedType} onValueChange={setSelectedType}>
                <SelectTrigger>
                  <SelectValue placeholder="Distillation type" />
                </SelectTrigger>
                <SelectContent>
