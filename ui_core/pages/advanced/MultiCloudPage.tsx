  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <div className="grid gap-4 md:grid-cols-4">
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm">Best Provider</CardTitle>
          </CardHeader>
          <CardContent>
            <p className="text-2xl font-bold">
              {bestProvider?.provider_name.toUpperCase() || "N/A"}
            </p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm">Failovers (24h)</CardTitle>
          </CardHeader>
          <CardContent>
            <p className="text-2xl font-bold">{failoverLogs.length}</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2">
          ...
          <CardContent>
            <p className="text-2xl font-bold">{routingRules.filter((r) => r.is_active).length}</p>
          </CardContent>
        </Card>
      </div>
      ...
      <Tabs defaultValue="providers">
        ...
        <TabsContent value="providers" className="space-y-4">
          <Card>
            ...
            <CardContent className="flex gap-4">
              ...
              <Select value={newRegion} onValueChange={setNewRegion}>
                ...
                <SelectContent>
                  {(REGIONS[newProvider as keyof typeof REGIONS] || []).map((r) => (
          ...
          {providers.length === 0 ? (
            <EmptyState
              title="No providers"
              description="Add cloud providers to enable multi-cloud routing"
              icon={Cloud}
            />
          ) : (
            <div className="grid gap-4 md:grid-cols-3">
              {providers.map((provider) => {
                ...
                const providerInfo = CLOUD_PROVIDERS_LIST.find(
                  (p) => p.value === provider.provider_name,
                );
                return (
                  <Card key={provider.id}>
                    <CardHeader>
                      <div className="flex items-center justify-between">
                        <CardTitle className="flex items-center gap-2">
                          {providerInfo?.icon} {provider.provider_name.toUpperCase()}
                        </CardTitle>
                        <Switch
                    ...
                    <CardContent className="space-y-3">
                      <div className="grid grid-cols-2 gap-2 text-sm">
                        <div>
                          ...
                          <p className="font-medium">
                            {stats.avgLatency ? `${stats.avgLatency.toFixed(0)}ms` : "N/A"}
                          </p>
                        </div>
                        <div>
                      ...
                      <div className="flex gap-2">
                        <Badge variant={provider.credentials_configured ? "default" : "outline"}>
                          {provider.credentials_configured ? "Configured" : "Not Configured"}
                        </Badge>
                      </div>
                      <Button
                        size="sm"
              ...
              <CardHeader className="pb-2">
                <div className="flex items-center justify-between">
                  ...
                  <Switch
                    ...
                    onCheckedChange={(checked) => toggleRoutingRule(rule.id, checked)}
                  />
                </div>
              </CardHeader>
              <CardContent>
                <Badge>
                  {ROUTING_MODES.find((m) => m.value === rule.mode)?.label || rule.mode}
                </Badge>
              </CardContent>
            </Card>
          ))}
