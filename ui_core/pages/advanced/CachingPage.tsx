...
const CachingPage = () => {
  ...
  const handleWarm = async () => {
    await startWarmingJob({
      job_type: "manual",
      target_cache_level: selectedLevel,
      items_to_warm: 100,
      trigger_reason: "Manual warming",
    });
  };

  if (isLoading) return <LoadingState message="Loading cache data..." />;

  const hitRateData = CACHE_LEVELS.map((l) => ({
    name: l.value,
    hitRate: getHitRateByLevel(l.value),
    size: getTotalSizeByLevel(l.value) / (1024 * 1024),
  }));
          ...
          <Card key={level.value}>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground">
                Hit Rate • {(getTotalSizeByLevel(level.value) / (1024 * 1024)).toFixed(2)} MB
              </p>
              <p className="text-xs text-muted-foreground mt-1">
                {level.policy} • TTL: {level.ttl}
              </p>
            </CardContent>
          </Card>
        ))}
      ...
      <Tabs defaultValue="overview">
        ...
        <TabsContent value="overview" className="space-y-4">
          <div className="grid gap-4 md:grid-cols-2">
            <Card>
              <CardHeader>
                <CardTitle>Hit Rate by Level</CardTitle>
              </CardHeader>
          ...
          <Card>
            ...
            <CardContent>
              {topQueries.length === 0 ? (
                <EmptyState title="No cached queries" />
              ) : (
                    ...
                    >
                      <div>
                        <p className="font-medium font-mono text-sm">
                          {query.cache_key.slice(0, 50)}...
                        </p>
                        <p className="text-xs text-muted-foreground">
                          {query.cache_level} • {query.content_type}
                        </p>
                      </div>
        ...
        <TabsContent value="entries">
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="space-y-2">
                {cacheMetadata
                  .filter((c) => c.cache_level === selectedLevel)
                  .slice(0, 10)
                  .map((entry) => (
                    <div
                      key={entry.id}
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-muted-foreground">
                          {((entry.size_bytes || 0) / 1024).toFixed(1)} KB
                        </span>
                      </div>
                    </div>
                  ))}
          ...
          <Card>
            ...
            <CardContent>
              {warmingJobs.length === 0 ? (
                <EmptyState title="No warming jobs" />
              ) : (
                warmingJobs.map((job) => (
                  <div
                    key={job.id}
                  ...
                  >
                    <div>
                      <p className="font-medium">
                        {job.job_type} - {job.target_cache_level}
                      </p>
                      <p className="text-sm text-muted-foreground">{job.trigger_reason}</p>
                    </div>
                    <div className="flex items-center gap-4">
                      <Progress
                        value={(job.items_warmed / (job.items_to_warm || 1)) * 100}
                        className="w-24"
                      />
          ...
          <Card>
            ...
            <CardContent>
              {invalidationLogs.length === 0 ? (
                <EmptyState title="No invalidation logs" />
              ) : (
                invalidationLogs.map((log) => (
                  ...
                  >
                    <div>
                      <p className="font-medium">
                        {log.invalidation_type} on {log.cache_level}
                      </p>
