...
class ApproxEngine:
    def approximate(self, query: str, canonical: Optional[str] = None) -> Optional[Dict[str, Any]]:
        ...
        # Point 7: Ultra-fast path for system stress
        if mode != ChaosMode.NORMAL:
             logger.info("approx_engine: SYSTEM STRESS. Using template-only approximation.")
             return {
                 "answer": f"System is under heavy load. Returning cached core reference for '{entity}'. Full detailed analysis is processing in the background.",
                 "confidence": 0.70
             }
