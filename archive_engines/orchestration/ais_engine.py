...
try:
    ...
    from archive_engines.approximation.probabilistic import ProbabilisticCore # type: ignore
except (ImportError, ValueError):
    try:
        from archive_engines.orchestration.identity import IdentityMapper # type: ignore
        from archive_engines.orchestration.hyper_engine import HyperIntentEngine # type: ignore
        from archive_engines.orchestration.compressed_dag import CompressedLatticeDAG # type: ignore
        from archive_engines.orchestration.outcome_lookup import OutcomeLookup # type: ignore
        from archive_engines.orchestration.symbolic_core import SymbolicAICore # type: ignore
        from archive_engines.approximation.probabilistic import ProbabilisticCore # type: ignore
    except (ImportError, ValueError):
