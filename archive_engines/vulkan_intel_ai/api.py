...
async def handle_vulkan_query(raw_input: Dict[str, Any]):
    ...
    try:
        ...
        query = str(raw_input.get("query", "")) if isinstance(raw_input, dict) else ""
        ...
        async def run_vulkan_pipeline():
            try:
                ...
                for i, task in enumerate(tasks):
                    yield json.dumps({"step": f"EXECUTING_{i+1}", "domain": task.get('domain', 'general')})
                    res = await kernel.execute_task(task, query)
                    results.append(res)
                    memory.scratchpad["intermediate_results"].append(res)
                    yield json.dumps({"step": f"RESULT_{i+1}", "summary": str(res)[:100]})
